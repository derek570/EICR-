...
- **Slice 5.5 — strangler writer + reader migration for the board / supply / installation surface.** Closes the loop on writes (slice 5.5.1 + 5.5.2) and serialiser reads (slice 5.5.3) so a flag-on session round-trips through `boards[currentBoardId]` BoardInfo for board-level fields and through composite-keyed `circuits['${id}::${ref}']` for per-circuit fields. Three commits.

  - **5.5.1 (`afd928f`)** — Added `applyBoardReadingMultiBoard` and `applyBoardReadingFlagAware` to `src/extraction/stage6-snapshot-mutators.js`. The new mutator writes to `snapshot.boards.find(b => b.id === resolvedId)` rather than `snapshot.circuits[0]`; on first write to a previously-unseen board id (which `ensureMultiBoardShape` doesn't pre-seed), it synthesises a minimal-viable BoardInfo (`{id, designation: id, board_type: 'sub-distribution' | 'main'}`) so a future writer pointing at an unbooked id never silent-drops. Flag-aware wrapper picks legacy `applyBoardReadingToSnapshot` (writes `circuits[0]`) under flag-off vs the new mutator under flag-on. **Why a separate mutator from `applyReadingMultiBoard`** (slice 5.2): the storage shape is different — circuits live at composite-keyed `snapshot.circuits['${id}::${ref}']`; board fields live at `snapshot.boards[].find(...)`. Sharing a helper would conflate two namespaces the iOS model deliberately separates. 8 new unit tests in `stage6-snapshot-mutators-multi-board.test.js` (active-board write / explicit boardId / currentBoardId fallback / synthesise sub-distribution / synthesise main / accrete fields / last-write-wins / no-collision with circuits[0]) + 3 new flag-aware wrapper tests in `stage6-multi-board-flag-routing.test.js`. Suite 73/73 green; full stage6 suite 1683/1683 green.

  - **5.5.2 (`4df60ee`)** — `dispatchRecordBoardReading` (`src/extraction/stage6-dispatchers-board.js`) now calls `applyBoardReadingFlagAware` and threads `input.board_id` through. Schema doesn't yet expose `board_id` (Phase 6 / Codex deal-breaker #3 widens the `board_ops` wire channel + tool surface), so today the wrapper falls back to `snapshot.currentBoardId` on every call — but the dispatcher is forward-compatible. **The path-2 hotspot:** the auto-resolve hook (`createAutoResolveWriteHook` in `stage6-dispatchers.js`) dispatches synthetic `record_board_reading` calls when an `ask_user` reply resolves a `pending_write`. Today those land at `circuits[0]`. Under flag-on with this slice, they land at `boards[currentBoardId]` instead. The resolver itself is unchanged — only the dispatcher's write surface shifts. The path-2 invariants (memory/handoff_2026-04-27_path2_review_fixes.md) are preserved. 3 new flag-on round-trip tests: happy path, forward-compatible `input.board_id`, and a path-2 synthetic-call invariant pin (`tool_call_id` containing `::auto::` lands on BoardInfo and is `auto_resolved`-tagged in `perTurnWrites`). 71/71 flag-routing + schema tests green; 146/146 path-2 resolver tests green; 174/174 dispatcher tests green.

  - **5.5.3 (`68d7b6e`)** — Migrated three reader sites in `buildStateSnapshotMessage` (`src/extraction/eicr-extraction-session.js`): supply-line source switched from `circuits[0]` to `boards[currentBoardId]` BoardInfo (with skeleton keys `id` / `designation` / `board_type` filtered so the emitted snapshot stays byte-equivalent to the legacy shape — only the data SOURCE changes, not the format); non-supply iteration replaced with `listCircuitRefsInBoard`; per-ref bucket fetch replaced with `getCircuitBucket` (with the self-describing `circuit` + `board_id` keys elided from the compact-key projection so per-circuit JSON lines stay byte-equivalent). `hasCircuits` widened to `hasSupply || circuits has data` so a flag-on session that only ever called `record_board_reading` (BoardInfo populated, circuits keyed object empty) still emits a snapshot. The token-estimate log line at the function tail uses the same flag-aware iteration. **Without this, flag-on board writes would land on BoardInfo but the snapshot would still read from the empty circuits[0] — Sonnet would never see Ze, PFC, etc.** 7 new tests in `eicr-extraction-session.snapshot-refactor.test.js` (flag-off regression guard, flag-on supply-line + skeleton-key filter, flag-on empty BoardInfo emits no `0:` line, flag-on per-circuit lines elide self-describing fields, currentBoardId scoping for non-supply iteration, mid-session board switch surfaces sub-board scope, defensive null-boards return-null path). Snapshot byte-equivalence under flag-off pinned by the 36 pre-existing tests in this file (none modified). Test counts: 3057 total → 3054 passed + 3 skipped.

  Slice 5.6 (legacy `circuits[0]` removal + retire `*FlagAware` wrappers + drop `STAGE6_MULTI_BOARD` env gate) is **deliberately held back** until production has soaked under `STAGE6_MULTI_BOARD=true` for ≥1 deploy cycle — the path-2 resolver is the highest-risk regression surface and we want CloudWatch traffic, not just CI, to confirm flag-on parity before destroying the legacy bucket. Phase 8.4 is the field-test gate that flips the env var on; slice 5.6 follows that.

- **Phase 6.0 — Stage 6 board-ops wire channel.** Closes Codex deal-breaker #3 from the multi-board planning review. Two commits.

  - **Backend (`2706123`)** — `createPerTurnWrites()` (`src/extraction/stage6-per-turn-writes.js`) seeds an empty `boardOps: []` array alongside the existing slots. The bundler (`src/extraction/stage6-event-bundler.js`) emits a new `board_ops` slot on the wire result when the accumulator is non-empty; OMITTED when empty so pre-Phase-6 traffic (every session today, since no dispatcher writes here yet) stays byte-identical to before. Discriminated by `op` field (`add_board` / `select_board` / `mark_distribution_circuit` and any future board-mutation tool); each entry passes through verbatim with a defensive shallow copy so downstream mutation cannot retro-alter the bundled result. Mirrors the existing `circuitOps` pattern. Optional in the input shape so older test fixtures predating the slot still round-trip cleanly. 4 new tests across `stage6-per-turn-writes.test.js` and `stage6-event-bundler.test.js`: factory seeds the array, fresh-object isolation extends to boardOps, non-empty slot emits verbatim, discriminated-union shape contract (every entry has a string `op` field), defensive shallow copy on emit, missing-key tolerance. Backend suite 3058/3061 green (was 3054 baseline pre-6.0).

  - **iOS (`3734b67`)** — New `BoardOp` Codable struct in `Sources/Services/ClaudeService.swift` and a `boardOps: [BoardOp]?` slot on `RollingExtractionResult` mapped to wire field `board_ops`. The struct uses all-Optional payload fields (`boardId` / `designation` / `boardType` / `parentBoardId` / `circuitRef` / `feedCircuitRef` / `feedsBoardId`) discriminated by a `String` `op` field. `decodeIfPresent` keeps every existing payload decoding cleanly. **Forward-compat critical:** `op` is `String` (not enum) so a future server-side op (e.g. `delete_board` / `rename_board`) decodes without throwing — iOS can ship before its UI handler does. The matching XCTest `testResponseParsingToleratesUnknownBoardOp` pins this contract. WHY this struct shape rather than enum-with-associated-values: an enum union would have demanded a custom `init(from:)` switching on `op` and would have rejected unknown ops with a Codable error rather than tolerating them — bad for forward compat across iOS releases. 6 new XCTests in `ClaudeServiceTests` covering absent slot (every session today), all 3 known op cases, mixed-op array preserving order, and unknown-op forward-compat. xcodebuild build green via `generic/platform=iOS Simulator`; 36/36 ClaudeServiceTests pass via Mac Catalyst (no iOS simulators installed on this Mac, same pattern Phases 1-4 used).

  WHY land the wire channel now, before the tools: Phase 6 will add `add_board` / `select_board` / `mark_distribution_circuit` dispatchers; without the channel, those dispatchers would write to a slot that doesn't exist on the wire and the iOS client would silently drop every board mutation. Channel-first lets a Phase 6 dispatcher's first synthetic write flow end-to-end the moment it ships, with no coordinated cross-PR change to the wire shape.

  Production behaviour is unchanged — the channel is inert until Phase 6 dispatchers populate it (accumulator stays empty, bundler omits the slot, iOS decoder surfaces `nil`). Phase 6 (the tools themselves) and Phase 7 (system prompt + ask-user resolver multi-board awareness) are the next concrete steps. Phase 6 is gated only on Phase 6.0 — slice 5.6 is independent (legacy `circuits[0]` removal, gated on production soak under `STAGE6_MULTI_BOARD=true`).
