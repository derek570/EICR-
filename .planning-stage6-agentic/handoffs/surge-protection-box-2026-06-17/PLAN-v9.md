...
- `config/baseline_config.json:24-29` + `config/eic_baseline_config.json:22-27` — both seed a nested `supply_characteristics.supply_protective_device` but no surge. Add a `surge_protection_device` seed dict (`present:'N/A'`, `type:'N/A'`, `bs_en:'N/A'`, `status_indicator:'N/A'`) to BOTH (loaded by `src/process_job.js:225` + `eicr_editor.py`) so the default supply shape includes surge.
...
- **Reconcile drifted doc-extraction prompts** — `config/prompts/sonnet_extraction_system.md:243-246` + `sonnet_extraction_eic_system.md:196-199` currently describe `spd_bs_en` as "BS EN 61643-11" (surge). Repoint those to the cutout meaning AND add the new `surge_*` keys so GPT-Vision document extraction populates the right family. (Also `config/prompts/extraction_system.md:29-32` already says cutout — keep.) **ALSO:** the SAME doc-extraction prompts route "main fuse"/"supply fuse"/"cutout" EXAMPLES to `main_switch_bs_en`/`main_switch_current` (`sonnet_extraction_system.md:131-132,219-220`; `sonnet_extraction_eic_system.md:84-85,181-182`) — contradicting Option A. Grep BOTH prompts for ALL `main fuse`/`supply fuse`/`cutout` guidance and repoint cutout/main-fuse BS+current → `spd_bs_en`/`spd_rated_current` (consumer isolator stays `main_switch_*`; surge only → `surge_*`). Add a doc-extraction regression for "main fuse BS 1361, 100 amp" proving it does NOT populate `main_switch_*`.
- **ALL extraction prompts/schemas with a supply shape (grep-ALL — do this ONCE, exhaustively, so /ep doesn't discover them one at a time).** Beyond the config doc-prompts above, several HARDCODED schemas carry their own `supply_characteristics` shape. Grep `config/prompts/*.md` AND `src/**/*.js` (RECURSIVE — INCLUDING `src/routes/`) for `main_switch_bs_en` / `supply_characteristics` / `supplyFillFields`; for EVERY hit that lists `main_switch_*` but omits `spd_*`/`surge_*`, either (a) add the 4 `surge_*` keys + `spd_bs_en`/`spd_rated_current` + the supply-fuse-vs-surge disambiguation, or (b) if the path is confirmed dead legacy, document it as off all live clients and skip (same treatment as `src/generate_pdf.js`). Where a hit is a hardcoded supply ALLOWLIST, prefer deriving it from `field_schema.json` supply fields so future additive fields don't silently drop. KNOWN hits as of this plan:
  - `src/extract_chunk.js` `SYSTEM_PROMPT` (~:16-54, `/api/recording`);
  - `src/ocr_certificate.js` `OCR_SYSTEM_PROMPT` + default supply shape (~:20-131, `/api/ocr`);
  - `src/extract_session.js` `SESSION_PROMPT` (~:31-85, `/api/recording/extract-transcript`);
  - `config/prompts/sonnet_text_system.md` (via `src/sonnet_extract.js:31` → `ws-recording.js` / `/api/recording/sonnet-extract`).
  - **`src/routes/extraction.js:3351` — the LIVE `/api/analyze-document` hardcoded supply schema** (has `spd_*`/`main_switch_*`, NO `surge_*`). This is an ACTIVE document/photo path (not legacy) — add the 4 `surge_*` keys + enum/guidance + disambiguation here for sure. Add an analyze-document regression proving a photographed "Type 2 surge device" lands in `surge_*`.
  - `src/routes/recording.js:402` — legacy `/api/recording` finish merge uses a hardcoded `supplyFillFields` allowlist that drops `surge_*`. Derive it from `field_schema.json` supply fields, or document the route dead.
  - **NOTE:** current iOS + web both record via the Stage 6 `/api/sonnet-stream` path, so the `/api/recording/*` routes + `sonnet_text_system.md` + `extract_session.js`/`sonnet_extract.js` appear to be LEGACY with no live client — VERIFY, then either fix-for-completeness or explicitly document as dead. `extract_chunk.js` + `ocr_certificate.js` (`/api/recording`, `/api/ocr`) are the likely-live ones; prioritise those.
  - Add route/unit tests for "Type 2 surge protection fitted, indicator OK" AND "main fuse BS 1361, 100 amp" (the latter must NOT populate `main_switch_*`) on whichever paths stay live.
...
- **`SchemaCoverageRescueTests.swift` MUST be edited in the same change (else the iOS suite goes red and fails the green gate).** It has a HARDCODED `mustBeRescued` set listing `spd_type` (`:56`) and asserts `mustBeRescued.subtracting(supplyFields)` is empty — when `spd_type` leaves `supplyFields`, remove it from `mustBeRescued` too. If instead routing `spd_type` into `deprecatedSupplyAliases`, bump that file's `deprecatedSupplyAliases.count == 6` assertion + message (`:82`). Separately, when ADDING the 4 `surge_*` keys to `supplyFields`, also add them to `mustBeRescued` to preserve the suite's silent-drop coverage intent.
...
- `CertMateUnified/Sources/ViewModels/LiveFillState.swift:154-157` — `displayedLengths` (drives the live-fill highlight read by `LiveFillView` `key:`/`state:`) tracks `spd*` but not surge. Add the 4 surge `displayedLengths` entries using the SAME key strings the new LiveFillView surge `LiveField`s pass, or the surge live rows get no fill-tracking and the keys won't match. (The §3b LiveFillView work depends on this file too — name it explicitly.)
- `CertMateUnified/Sources/Services/CertificateDefaultsService.swift:335-338` — `applyPreset`/`applyStandardDefaults` (live, called from `JobListViewModel:220,234`) copies supply via an EXPLICIT per-field list ending at the `spd*` lines. Add the 4 `surgeSpd*` copies (`if dst.surgeSpdPresent.isNilOrEmpty { dst.surgeSpdPresent = src.surgeSpdPresent }`, etc.) right after, or preset/defaults-sourced surge values are silently dropped (iOS-only asymmetry — the web defaults service iterates generically). Add a defaults-service test.
...
- **Web live-fill (recording surface):** `web/src/components/live-fill/live-fill-view.tsx:140` + `:244` render mirrored `board_info.spd_*` as "SPD BS EN" / "SPD type" / "SPD rating" DURING recording — relabel to "Main Fuse" / "Supply Protective Device" and add separate `surge_*` live rows (web twin of the iOS `LiveFillView` work).
- **Web job overview:** `web/src/app/job/[id]/page.tsx:119` — the overview "Main Fuse" card reads NONEXISTENT `supply.main_fuse_*` aliases then falls back to `board.main_switch_*`, so it shows the consumer-unit main switch in the Main Fuse box and IGNORES the real cutout in `supply.spd_*`. Rebind those rows to `supply.spd_bs_en` / `supply.spd_rated_current`. Add a web overview render test.
- **Web defaults preset editor:** `web/src/components/defaults/preset-editor-sheet.tsx:50` — `SupplyDraft`/`pickSupply`/`emitDraft` is a hardcoded supply field set; existing presets' `surge_*` values DROP on edit/save. Either add the 4 surge controls to `SupplyDraft`, or preserve unknown supply keys through the round-trip and merge them back in `emitDraft`. Add a test proving an existing `surge_spd_type` survives an unrelated preset edit.
- **Excel export:** `src/export.js:123-137` `SUPPLY_HEADERS` (used at `:251` `objectToRow(supply, SUPPLY_HEADERS)`) emit "SPD BS EN" / "SPD Type" / "SPD Rated Current" and OMIT `surge_*` — rename those supply headers to Main Fuse / Supply Protective Device AND append 4 new `surge_*` columns, or downloads still mislabel main-fuse data and drop surge data. The work is ENTIRELY in `src/export.js`; `src/routes/export.js` only routes the endpoint (nothing to change there). Add an export regression test. (Backend `src/` change — rides Slice B/D, deploy via CI.)
...
- Server-side (used by the web frontend, NOT the iOS app — `CertMateUnified/CLAUDE.md:28-30`). The data flow has THREE layers — ALL THREE must change or the surge block renders blank (the generator reads a NESTED dict, not the flat keys):
  ...
  2. `python/generate_full_pdf.py:287-304` (nesting layer, ~:299) — builds the nested `supply_characteristics` dict. Add a nested `surge_protection_device` dict alongside the existing `supply_protective_device`, mapping `surge_spd_present`/`surge_spd_type`/`surge_spd_bs_en`/`surge_status_indicator` from `board_details.json`. (CONFIRMED-LIVE separate app: `python/eicr_editor.py` is the separately-deployed Streamlit editor — `streamlit run python/eicr_editor.py`. Because §3d.3 edits the SHARED generator to render `supply.get('surge_protection_device', {})`, the editor will otherwise render a BLANK, unfillable surge block. So: add a "Surge Protection Device" INPUT section to the editor UI mirroring the Supply Protective Device section (~:2514-2547), AND add a nested `surge_protection_device` dict to its own supply build (~:3962-3978) using the editor's OWN session keys — `spd_type`/`spd_capacity`/`spd_current`-style — NOT a copy of the `generate_full_pdf` mapping. If the editor is being retired, document it out-of-scope instead.)
