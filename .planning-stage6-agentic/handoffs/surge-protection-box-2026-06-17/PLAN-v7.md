...
- **ALL extraction prompts/schemas with a supply shape (grep-ALL — do this ONCE, exhaustively, so /ep doesn't discover them one at a time).** Beyond the config doc-prompts above, several HARDCODED schemas carry their own `supply_characteristics` shape. Grep `config/prompts/*.md` AND `src/*.js` for `main_switch_bs_en` / `supply_characteristics`; for EVERY hit that lists `main_switch_*` but omits `spd_*`/`surge_*`, either (a) add the 4 `surge_*` keys + `spd_bs_en`/`spd_rated_current` + the supply-fuse-vs-surge disambiguation, or (b) if the path is confirmed dead legacy, document it as off all live clients and skip (same treatment as `src/generate_pdf.js`). KNOWN hits as of this plan:
...
- **Excel export:** `src/export.js:127` `SUPPLY_HEADERS` + `src/routes/export.js:45` emit "SPD BS EN" / "SPD Type" / "SPD Rated Current" and OMIT `surge_*` — rename those supply headers to Main Fuse / Supply Protective Device AND append 4 new `surge_*` columns, or downloads still mislabel main-fuse data and drop surge data. Add an export regression test. (Backend `src/` change — rides Slice B/D, deploy via CI.)
