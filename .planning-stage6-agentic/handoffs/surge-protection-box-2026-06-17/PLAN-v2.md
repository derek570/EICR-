...
- iOS voice path leak: `applySonnetReadings` has `case "spd_type_supply", "spd_type":` (`DeepgramRecordingViewModel.swift` ~5970) folding the board-scoped `spd_type` key INTO the supply `spd_type_supply` write. Decide whether `spd_type` should still write supply — if it carries surge/board data, drop the `"spd_type"` alias from that case head or route it to `surge_*`.
...
- Server-side (used by the web frontend, NOT the iOS app — `CertMateUnified/CLAUDE.md:28-30`). The data flow has THREE layers — ALL THREE must change or the surge block renders blank (the generator reads a NESTED dict, not the flat keys):
  1. `src/routes/pdf.js:101-104` — flattens supply keys into `board_details.json`; add the 4 flat `surge_*` keys to that pass-through.
