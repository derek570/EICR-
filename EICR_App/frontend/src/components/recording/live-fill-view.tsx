...
function FieldRow({
  ...
  return (
    <div className="flex gap-2 min-h-[20px]">
      <span className="text-xs text-zinc-500 w-24 shrink-0 text-right">
        {label}:
      </span>
...
function SectionCard({
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-3">
      <h3 className="text-xs uppercase text-zinc-500 tracking-wider font-semibold mb-2">
        {title}
      </h3>
...
function InstallationSection({
  ...
  return (
    <SectionCard title="Installation">
      ...
      <FieldRow
        label="Next Insp."
        value={install?.next_inspection_years ? `${install.next_inspection_years} years` : undefined}
...
function EarthingSection({
  ...
  return (
    <SectionCard title="Earthing">
      <FieldRow
        label="Distributor"
        value={supply?.means_earthing_distributor != null ? (supply.means_earthing_distributor ? "Yes" : "No") : undefined}
        isRecording={isRecording}
      />
      <FieldRow
        label="Electrode"
        value={supply?.means_earthing_electrode != null ? (supply.means_earthing_electrode ? "Yes" : "No") : undefined}
        isRecording={isRecording}
      />
      <FieldRow label="Earth Mat." value={supply?.earthing_conductor_material} isRecording={isRecording} />
      <FieldRow
        label="Earth CSA"
        value={supply?.earthing_conductor_csa ? `${supply.earthing_conductor_csa}mm\u00B2` : undefined}
        isRecording={isRecording}
      />
      <FieldRow label="Bond Mat." value={supply?.bonding_conductor_material} isRecording={isRecording} />
      <FieldRow
        label="Bond CSA"
        value={supply?.bonding_conductor_csa ? `${supply.bonding_conductor_csa}mm\u00B2` : undefined}
        isRecording={isRecording}
      />
    </SectionCard>
  );
            ...
            return (
              ...
              >
                ...
                <span className="w-10 text-right text-zinc-400 font-mono">
                  {c.ocpd_rating_a ? `${c.ocpd_rating_a}A` : <span className="text-zinc-600">&mdash;</span>}
                </span>
              </div>
            );
          })}
