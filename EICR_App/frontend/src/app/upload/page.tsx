      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="mb-6">
          <CardHeader>
            ...
            <CardDescription>Add audio recordings and photos of the consumer unit</CardDescription>
          </CardHeader>
          <CardContent>
            {/* Drop zone */}
            <div
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {files.map((f) => (
                    <div key={f.id} className="flex items-center gap-3 p-3 bg-muted rounded-lg">
                      {f.preview ? (
                        <img src={f.preview} alt="" className="h-12 w-12 object-cover rounded" />
                      ) : (
                        getFileIcon(f.file)
                      )}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">{formatSize(f.file.size)}</p>
                      </div>
        ...
        <Card className="mb-6">
          <CardHeader className="cursor-pointer" onClick={() => setShowOcrImport(!showOcrImport)}>
                  ...
                  {ocrData.installation_details?.address && (
                    <div className="p-3 bg-muted rounded-lg space-y-1">
                      ...
                      {ocrData.installation_details.client_name && (
                        <p className="text-sm">
                          Client: {ocrData.installation_details.client_name}
                        </p>
                      )}
                    </div>
                  )}
                      ...
                      <div className="space-y-1 max-h-40 overflow-y-auto">
                        {ocrData.circuits.map((c, i) => (
                          <div key={i} className="flex items-center gap-2 text-sm">
                            ...
                            {c.measured_zs_ohm && (
                              <span className="text-muted-foreground">Zs:{c.measured_zs_ohm}</span>
                            )}
                          </div>
                        ))}
                      </div>
                        ...
                        {ocrData.observations.map((obs, i) => (
                          <div key={i} className="flex items-start gap-2 text-sm">
                            ...
                            <span className="truncate">
                              {obs.observation_text || obs.item_location}
                            </span>
                          </div>
                        ))}
                      </div>
