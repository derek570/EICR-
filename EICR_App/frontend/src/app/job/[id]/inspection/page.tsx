...
function findLinkedObservation(
...
export default function InspectionPage() {
  ...
  const updateFlag = (
    flag: 'markSection7NA' | 'hasMicrogeneration' | 'isTTEarthing',
    value: boolean
  ) => {
    updateJob({
      inspection_schedule: {
    ...
    return (
      <div className="p-4">
        <p className="text-muted-foreground">
          This page is for EICR certificates. Use the EIC Inspection tab for EIC certificates.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <Card>
        <CardContent className="pt-4 space-y-4">
          ...
          <div className="flex items-center gap-3 pb-3 border-b">
            ...
            <span className="text-xs text-muted-foreground">
              {hasMicrogeneration
                ? '(2.0, 4.11, 4.21, 4.22 ticked)'
                : '(2.0, 4.11, 4.21, 4.22 N/A)'}
            </span>
          </div>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm">
              Mark ALL Section 7 items as N/A (special locations not present)
            </span>
          </label>
        </CardContent>
      </Card>
              ...
              <CardContent className="pt-0">
                <div className="divide-y">
                  {Object.entries(items).map(([itemId, description]) => {
                    ...
                    const disabled =
                      (isSection7Item && markSection7NA) || isMicrogenerationItem || isEarthingItem;
      ...
      <div className="text-xs text-muted-foreground space-y-1">
        <p>
          <span className="font-medium">{'\u2713'}</span> = Inspected and satisfactory
        </p>
        <p>
          <span className="font-medium">N/A</span> = Not applicable
        </p>
        <p>
          <span className="font-medium text-red-600">C1</span> = Danger present - requires urgent
          attention
        </p>
        <p>
          <span className="font-medium text-orange-600">C2</span> = Potentially dangerous - requires
          improvement
        </p>
        <p>
          <span className="font-medium text-blue-600">C3</span> = Improvement recommended
        </p>
        <p>
          <span className="font-medium text-yellow-600">LIM</span> = Limitation - unable to inspect
        </p>
      </div>
    </div>
  );
