...
// 0a. "Nought 88" → "0.88"
const impliedDecimalRe = new RegExp(
  `\\b(naught|nought|zero|oh)\\s+(\\d{2,3})\\b`, 'gi'
);

// 0b. "naught 0.14" → "0.14"
const strayDigitWordRe = new RegExp(
  `\\b(naught|nought|zero|oh|no)\\s+(?=\\d+\\.)`, 'gi'
);

// 0c. "Nought0.87" → "0.87"
const gluedDigitWordRe = new RegExp(
...
const impliedZeroDecimalRe = new RegExp(
  `(?<=\\s|^)point\\s+(${DW})(?:\\s+(${DW}))?\\b`, 'gi'
);

// 5. "twenty one" → "21"
const tensOnesRe = /\b(twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety)\s+(one|two|three|four|five|six|seven|eight|nine)\b/gi;

// 6. "thirteen" → "13"
const teensRe = /\b(ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen)\b/gi;
 ...
 */
function replaceWithFn(text: string, regex: RegExp, fn: (match: RegExpExecArray) => string | null): string {
  regex.lastIndex = 0;
...
export function normalise(text: string): string {
  ...
  // Must run before any other pattern to prevent pointDigitPattern (step 8b)
  // from converting "Point 0.87" → "0.0.87".
  ...
  // 0a. Convert implied decimals: "Nought 88" → "0.88" (speaker dropped "point")
  ...
  // 0b. Remove stray digit words before numeric values WITH decimal ("naught 0.14" → "0.14")
  strayDigitWordRe.lastIndex = 0;
  result = result.replace(strayDigitWordRe, '');

  // 0c. Handle zero-word glued to digits without space ("Nought0.87" → "0.87")
  ...
  result = replaceWithFn(result, decimalRe, (m) => {
    ...
    if (!w || !f1) return null;
    let decimal = `${w}.${f1}`;
    if (d2) { const f2 = digitWords[d2]; if (f2) decimal += f2; }
    if (d3) { const f3 = digitWords[d3]; if (f3) decimal += f3; }
    return decimal;
  });
  ...
  // 8. Collapse single-digit sequences: "2 9 9" → "299" (Deepgram ASR artefact)
  ...
  // 8b. "point 60" → "0.60" — standalone "point" before already-numeric digits
