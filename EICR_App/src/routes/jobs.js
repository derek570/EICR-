...
async function processJobAsync(userId, jobId) {
  try {
    ...
    if (storage.isUsingS3() && tempDir) {
      const folderName = sanitizeS3Path(result.address) || jobId;
      ...
      if (result.address && result.address !== jobId) {
        ...
        logger.info('Deleting old job folder after rename', {
          oldPrefix,
          newAddress: result.address,
        });
        const deleteResult = await storage.deletePrefix(oldPrefix);
        logger.info('Delete prefix result', {
          oldPrefix,
          deleted: deleteResult.deleted,
          errors: deleteResult.errors,
        });
      } else {
        logger.debug('Not deleting old folder', {
          address: result.address,
          jobId,
          reason: result.address === jobId ? 'address equals jobId' : 'no address',
        });
      }
  ...
  try {
    ...
    if (storage.isUsingS3()) {
      ...
      if (inputFiles.length === 0) {
        ...
        const audioPhotoFiles = altFiles.filter(
          (f) => /\.(m4a|mp3|wav|aac|jpg|jpeg|png|heic)$/i.test(f) && !f.includes('/output/')
        );
    ...
    if (storage.isUsingS3() && tempDir) {
      ...
      if (result.address && result.address !== jobId) {
        ...
        const deleteResult = await storage.deletePrefix(oldPrefix);
        logger.info('Old prefix cleanup complete', {
          deleted: deleteResult.deleted,
          errors: deleteResult.errors,
        });
      }
...
router.get('/job/:userId/:jobId/debug', auth.requireAuth, async (req, res) => {
  ...
  try {
    ...
    const debugContent = await storage
      .downloadText(`${s3Prefix}debug_transcription.json`)
      .catch(() => null);

    if (!debugContent) {
      return res.status(404).json({
        error:
          "Debug data not found. This job may have been processed before debug logging was enabled, or wasn't recorded through the real-time API.",
      });
    }
...
router.put('/job/:userId/:jobId', auth.requireAuth, async (req, res) => {
  ...
  try {
    ...
    try {
      const currentData = await storage
        .downloadText(`${s3Prefix}extracted_data.json`)
        .catch(() => null);
      if (currentData) {
        const changedFields = [];
        ...
        const summary = changedFields.length > 0 ? `Updated: ${changedFields.join(', ')}` : 'Saved';
    ...
    try {
      ...
      /* Ignore missing extracted_data.json — start fresh */
    }
    ...
    if (installation_details?.address) {
      const sanitizedAddress = sanitizeS3Path(installation_details.address);
      ...
      if (folderName !== sanitizedAddress && storage.isUsingS3()) {
        const oldPrefix = `jobs/${userId}/${folderName}/`;
        try {
          const newPrefix = `jobs/${userId}/${sanitizedAddress}/output/`;
          await storage.uploadText(
            JSON.stringify(extractedData, null, 2),
          ...
          logger.info('Migrated S3 data to new address folder', {
            jobId,
            oldFolder: folderName,
            newFolder: sanitizedAddress,
          });
        } catch (migrateErr) {
          logger.warn('Failed to migrate S3 folder on address change', {
            jobId,
...
router.post('/job/:userId/:jobId/clone', auth.requireAuth, async (req, res) => {
  ...
  try {
    ...
    let extractedData = {};
    const combinedJson = await storage
      .downloadText(`${sourcePrefix}extracted_data.json`)
      .catch(() => null);
    if (combinedJson) {
    ...
    const newPrefix = `jobs/${userId}/${sanitizeS3Path(newAddress.trim())}/output/`;
...
router.post('/jobs/:userId/bulk-download', auth.requireAuth, async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="certificates_${new Date().toISOString().split('T')[0]}.zip"`
    );
