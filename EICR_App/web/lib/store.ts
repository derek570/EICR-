...
export const useJobStore = create<JobState>((set, get) => ({
  ...
  updateCircuits: (circuits) => {
    ...
    saveLocalJob(buildLocalJob(updatedJob, userId));
  ...
  updateBoards: (boards) => {
    ...
    // Primary board info comes from first board
    const primaryBoard = boards[0];
    const flatBoardInfo = primaryBoard ? primaryBoard.board_info : currentJob.board_info;

    // Flatten all board circuits into a single array (multi-board support)
    const flatCircuits = boards.flatMap((b) => b.circuits);

    const updatedJob = {
      ...currentJob,
      boards,
      board_info: flatBoardInfo,
      circuits: flatCircuits.length > 0 ? flatCircuits : currentJob.circuits,
    };
    set({ currentJob: updatedJob, isDirty: true });
    saveLocalJob(buildLocalJob(updatedJob, userId));
    get().refreshPendingCount();
