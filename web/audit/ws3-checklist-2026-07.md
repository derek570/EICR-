...
| `src/extraction/stage6-tool-schemas.js` | #61, #66 | **iOS-companion-shipped** (obs wording) / **backend-only** (`context_board_id` — CORRECTED 2026-07-02) | Observation tool schemas gain canonical-wording plumbing (#66) → WS3 item 3. `ask_user.context_board_id` (#61) is BACKEND-INTERNAL: it feeds the server's ask-budget key + auto-resolve write sites; NEITHER client decodes it (verified 2026-07-02: zero refs in CertMateUnified Sources and `web/src/`). The client answer path is keyed by `tool_call_id` — web's matches iOS (`tests/transcript-gate-wiring.test.ts`). The original 'web should carry it the way iOS does' was wrong: iOS doesn't. |
...
5. ~~**`ask_user.context_board_id`** — carry board scope in web question handling like iOS (#61).~~ **RETRACTED 2026-07-02 (WS3 item 9c):** backend-internal — iOS does not decode the field either; the client answer path is `tool_call_id`-keyed on both platforms. No web action; ledger row `recording/ask-context-board-id` corrected to `match`.
