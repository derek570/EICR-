...
## Wave C — regex tier port (branch `pwa-regex-tier`, worktree at `../EICR_Automation_parity`)

Closes the two coupled Phase 6 P0s deferred from Wave B: "No regex layer" + "2-tier field priority instead of 3-tier". Plan: `web/audit/REGEX_TIER_PLAN.md`. Cut from `origin/main` so the work lands without depending on `pwa-parity-fixes`.
...
- Multi-line commit bodies: what / why / why-this-approach / test plan / vitest count.
- After every commit: `npm test --workspace=web -- --run` must stay green; `npx tsc --noEmit -p tsconfig.json` clean for modified files (the two pre-existing errors in `tests/phase-6-clear-cache.test.tsx` + `tests/phase-9-haptic.test.ts` are documented audit noise — ignore).
- Codex review per phase: `bash -lc 'codex review --commit <SHA>'`. Address P0/P1 findings before the next phase commit.
- Push `origin/pwa-regex-tier` after each phase. Open PR against `main` after R5 (so R6 soak runs against a real PR).
