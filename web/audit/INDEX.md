...
- ~~**No signature capture on web T&Cs gate (`06caaf9`)**~~ — **SHIPPED 2026-07-03 (WS7, ledger row `crosscutting/terms-signature-port`)**. Finger/pointer-drawn signature on the web terms gate, client-side storage (`termsAcceptanceSignature`), no backend write. The "await legal review" stance is fully superseded.
