...
export function useRecording(
  ...
  const [currentAlert, setCurrentAlert] = useState<ValidationAlert | null>(
    null,
  );
  ...
  const fieldSourcesRef = useRef<
    Record<string, "regex" | "sonnet" | "preExisting">
  >({});
  const jobRef = useRef<JobDetail | null>(null);
  const sonnetDebounceTimer = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const applyRegexResults = useCallback(
    (result: ReturnType<TranscriptFieldMatcher["match"]>) => {
      ...
      if (Object.keys(result.supplyUpdates).length > 0) {
        ...
        batchUpdate.supply_characteristics = supply as unknown as JobDetail["supply_characteristics"];
      }
      ...
      for (const [circuitRef, updates] of Object.entries(
        result.circuitUpdates,
      )) {
        if (!updates) continue;
      ...
      for (const reading of result.extractedReadings) {
        ...
        if (circuitRef === "0" || circuitRef === undefined) {
          ...
          const INSTALL_FIELDS = new Set([
            ...
            "premises_description", "next_inspection_years", "reason_for_report",
            "occupier_name", "date_of_previous_inspection", "previous_certificate_number",
            "estimated_age_of_installation", "general_condition",
            "agreed_limitations", "agreed_with", "operational_limitations", "extent",
          ]);
          ...
          if (section === "board") {
            ...
            install[field] = value;
            onJobUpdate({ installation_details: install as unknown as JobDetail["installation_details"] });
          } else {
            const supply = { ...currentJob.supply_characteristics } as Record<string, unknown>;
            supply[field] = value;
            onJobUpdate({ supply_characteristics: supply as unknown as JobDetail["supply_characteristics"] });
          }
        } else {
          // Circuit field
  ...
  const triggerSonnetExtraction = useCallback(async () => {
    ...
    const recentReadings = JSON.stringify(
      fieldSourcesRef.current,
    );
    const askedDescs =
      alertManager.current?.getAskedQuestionDescriptions() ?? [];
  ...
  const connectCompanionSocket = useCallback(
    (jobId: string, dgService: DeepgramService) => {
  ...
  const startRecording = useCallback(async (source?: AudioSource) => {
    ...
    try {
      ...
      const keywords = generateKeywordBoosts(
        job?.board_info,
        job?.circuits,
      );
      ...
      const dgDelegate: DeepgramDelegate = {
        ...
        onFinalTranscript(
          ...
          // Run regex matching
          if (jobRef.current) {
            const result = matcher.current.match(
              transcriptRef.current,
              jobRef.current,
            );
            ...
            debugLogger.current.debug("regex", "regex_attempt", {
              matches: Object.keys(result.circuitUpdates).length +
