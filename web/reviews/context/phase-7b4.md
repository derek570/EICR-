...
```
commit 1ec4e22934c384ddf240ef667d41cd18fd132ed0
Author:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
AuthorDate: Fri Apr 17 22:30:31 2026 +0100
Commit:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
CommitDate: Fri Apr 17 22:30:31 2026 +0100

    feat(web): Phase 7b iOS Add-to-Home-Screen hint (closes 7b)
    
    Final scope-exclusion item from PHASE_7A_HANDOFF.md §"Scope exclusions":
    closes Phase 7b. Without this, iPhone/iPad users have no pathway to
    discover that CertMate is installable — Safari never fires
    `beforeinstallprompt`, so the AppShell `<InstallButton />` stays hidden
    on iOS. A dismissible banner on `/settings` is the planned
    accommodation.
    
    Why here, why now:
      * On Chrome/Edge/Android, `beforeinstallprompt` is captured in
        `install-store.ts` and surfaced via the ghost/sm button in the
        AppShell header cluster. Safari desktop + iOS deliberately omit
        that event per Apple platform policy.
      * Without an explicit hint, an iOS inspector would never learn the
        app is installable — the only path is Share → Add to Home Screen
        → Add, and Safari gives no UI affordance pointing at it.
    
    Component — `web/src/components/pwa/ios-install-hint.tsx`:
      * Platform detection: `/iPad|iPhone|iPod/.test(ua) && !('MSStream' in
        window)`. The MSStream guard filters old Windows Phones that
        spoofed iOS user-agents; cost of keeping it is zero, cost of
        missing it is a wrong-hint on a long-tail device.
      * Already-installed check combines two signals so every iOS/iPadOS
        version is caught by whichever one it exposes reliably:
          - `(navigator as IOSNavigator).standalone === true` — iOS
            extension, cast locally rather than polluting global Navigator
            type.
          - `window.matchMedia('(display-mode: standalone)').matches` —
            the cross-browser standard, reliable on iPadOS 16+.
      * Dismissal persistence: localStorage key
        `cm_pwa_ios_hint_dismissed:v1`. The `:v1` suffix keeps us honest —
        "show once per user per campaign" means a future marketing push
        can reset the pool by bumping to `:v2`, no data-migration step.
        try/catch around setItem so private-mode Safari + quota-exceeded
        never throws into the root error boundary.
      * First-paint: `useState(false)` + effect-driven flip to true. SSR
        renders the no-show case and the client never flashes the banner
        for a frame on non-iOS during hydration.
    
    Visual design:
      * `<aside role="region" aria-label="Install CertMate on your iPhone">`
        — semantic landmark so screen readers can jump to or skip.
      * Brand-blue surface at 10% mix + 30%-opacity border — matches the
        6b LinkCard accent so the banner feels native to the settings
        surface, not a foreign nag.
      * Plus icon in a 10×10 rounded tile, matching the LinkCard icon
        treatment on the same page. Numbered ol with the Safari Share
        icon inline on step 1 (brand-blue, mirrors what the user will
        actually see in Safari's toolbar) and keyword-bolded "Add to
        Home Screen" / "Add" on step 2.
      * Dismiss × at top-right, 32×32 hit area with proper aria-label.
    
    Mount site — `web/src/app/settings/page.tsx`:
      * Placed between the hero profile section and the TEAM SectionGroup.
      * Deliberately NOT on the dashboard — the dashboard is already
        dense (hero + recent jobs + setup grid) and adding an install
        prompt would push Recent Jobs below the fold on a phone during
        the primary daily workflow.
      * `/settings` is low-traffic by inspector standards (once to
        configure, then rarely) — the user is already in "configure my
        app" mode and receptive to an install nudge.
      * Single import + single render site. The banner self-suppresses
        in every case it shouldn't render, so the page stays free of
        platform-specific branching; the show/hide decision lives in
        one place.
    
    Scope exclusions closed by this commit — all four 7b-tagged items
    from PHASE_7A_HANDOFF.md §"Scope exclusions" now landed:
      1. User-initiated SW update handoff (ce8323a)
      2. IDB read-through cache (2d3527f)
      3. AppShell offline indicator (a85487f)
      4. iOS ATHS hint (this commit)
    
    Still deferred: no outbox / mutation queue (7c — next phase), no
    offline job-edit UI polish (7d), no push/periodic-sync/share-target,
    no Sentry.
    
    Verification:
      * `npm run typecheck` clean (main + tsconfig.sw.json)
      * `npm run lint` at 0 errors / 6 warnings baseline (unchanged)
      * `npm run build --webpack` succeeds — `/settings` still prerenders
        static
    
    Manual DevTools:
      * Device emulation → iPhone 14 → `/settings` → banner visible on
        first visit
      * Tap × → banner hidden, localStorage row set
      * Reload → banner stays hidden
      * Application → clear `cm_pwa_ios_hint_dismissed:v1` → banner
        returns
      * Simulated Pixel 7 → banner never renders (UA regex miss)
...
```
 CLAUDE.md                                   |   1 +
 web/src/app/settings/page.tsx               |   9 ++
 web/src/components/pwa/ios-install-hint.tsx | 159 ++++++++++++++++++++++++++++
