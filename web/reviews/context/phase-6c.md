...
```
commit bc11914dc2250693b85492c15121e25558312e99
Author:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
AuthorDate: Fri Apr 17 19:58:12 2026 +0100
Commit:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
CommitDate: Fri Apr 17 19:58:12 2026 +0100

    feat(web): Phase 6c system-admin user management
    
    What:
    Final sub-phase of Phase 6. Ports the iOS admin CRUD surfaces
    (`AdminUsersListView` / `AdminEditUserView` / `AdminCreateUserView`)
    to the web client: new sub-tree at `/settings/admin/users` with list,
    create, edit, reset-password, and unlock flows. Five new API client
    methods; zero backend changes.
    
    Why:
    System admins previously had no way to manage users from the web —
    the 6a pre-work declared `AdminUser` and gated middleware but left
    the Manage Users link in a "Coming in Phase 6c" disabled state. This
    closes the loop so a web-only user has feature parity with iOS for
    lifecycle management (create, deactivate, reset, unlock) without
    needing to drop back to the iOS app.
    
    Why this approach:
    - Page files inline their forms instead of extracting a
      components/settings/admin-* tree. The dashboard in 6b set the
      precedent — settings pages are shallow enough that an abstraction
      would be premature, and the handoff explicitly says so.
    - `adminListUsers` always sends limit+offset so the backend returns
      the `Paginated<AdminUser>` envelope. Without params the backend
      falls back to a bare array; a consistent shape lets the list and
      the edit page share the same parsing path.
    - Edit page loads the row by scanning paginated pages (cap 1000 users)
      because the backend has no `GET /api/admin/users/:id`. 50-per-page
      hits on the first page almost always.
    - Self-edit guard client-side-disables role + is_active when editing
      self. The backend 400s at `admin-users.js:129-135` anyway; the
      client disable avoids a round-trip to discover the error and signals
      intent to the admin.
    - Reset-password is a modal sheet (copied from 6b `InviteEmployeeSheet`)
      so the main form doesn't balloon with a rarely-used field, and so
      the one-shot password never shares state with the row patch. No
      copy-to-clipboard reveal — the admin typed it, they don't need it
      read back. Success state surfaces "existing sessions signed out"
      because the backend increments the user's token version, killing
      every live JWT on next API call.
    - Unlock is inline on the Security SectionCard, conditional on
      `locked_until > nowMs`. `nowMs` comes from a
      `useState(() => Date.now())` init because `react-hooks/purity` rule
      disallows `Date.now()` directly in render. The value is stable per
      mount and the list re-fetches on window.focus, so an expired lockout
      clears within one refresh cycle.
    - No delete button. Backend has no delete endpoint and iOS doesn't
      surface one — `is_active: false` is the soft-delete path.
    - `LabelledSelect` built inline as a tiny primitive in the two pages
      that need it rather than adding a new UI component. If a third
      caller emerges we can extract later — premature abstraction is
      worse than a 30-line duplicate.
    
    Context:
    - Scope exclusions from handoff §"Scope exclusions" honoured: no
      delete UI, no audit-log surfacing (backend logs but no read endpoint
      — deferred to Phase 7+), no company-picker (raw UUID field only,
      full picker deferred), no bulk actions, no email notifications, no
      failed-login-attempts counter display (the Locked badge is enough
      signal).
    - Build verified clean: `tsc --noEmit` clean; `next build` lists
      `/settings/admin/users` + `/settings/admin/users/new` as static,
      `/settings/admin/users/[userId]` as dynamic. Lint at baseline:
      0 errors, 6 pre-existing warnings (unchanged from `6e85e9e`).
    - Phase 6 is now closed. Next candidates: PWA offline (Phase 7),
      Playwright E2E tests, or `web-rebuild` → `main` promotion.
...
```
 CLAUDE.md                                          |   1 +
 web/src/app/settings/admin/users/[userId]/page.tsx | 609 +++++++++++++++++++++
 web/src/app/settings/admin/users/new/page.tsx      | 269 +++++++++
 web/src/app/settings/admin/users/page.tsx          | 329 +++++++++++
 web/src/app/settings/page.tsx                      |   2 -
 web/src/lib/api-client.ts                          |  89 +++
 6 files changed, 1297 insertions(+), 2 deletions(-)
```

## Handoff doc: PHASE_6C_HANDOFF.md
