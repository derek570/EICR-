...
```
commit 2ef8ec6470c967078c5a9074e6c38c22256e83be
Author:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
AuthorDate: Fri Apr 17 18:40:53 2026 +0100
Commit:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
CommitDate: Fri Apr 17 18:40:53 2026 +0100

    feat(web): Phase 6a settings hub + inspector profiles
    
    WHAT
    Ports iOS settings surface to the web rebuild: a /settings hub, a
    /settings/staff list, and a shared /settings/staff/[inspectorId] add/edit
    page (with 'new' sentinel for creation). Adds signature capture via an
    HTML5 canvas + Pointer Events, equipment tracking (5 instruments × serial
    + calibration date), and a default-inspector mutex enforced client-side
    before the full-array PUT. Adds a backend GET route to read stored
    signature PNGs with bearer auth.
    
    WHY
    Inspectors need to manage their profile (name, position, organisation,
    enrolment number, signature, instruments) from the web the same way the
    iOS app lets them — every job PDF embeds the inspector's signature, and
    equipment serials + calibration dates flow into the EICR test-results
    page. Until now, web users had no way to edit these; they had to open
    the iOS app just to add staff. This also unblocks later phases (company
    settings, admin CRUD) which share the same settings layout scaffolding.
    
    WHY THIS APPROACH
    - Component-level useCurrentUser hook (hydrates from localStorage for
      instant first paint, revalidates via /api/me) + middleware JWT-decode
      as a belt-and-braces dual layer. Middleware blocks /settings/admin/*
      before HTML even leaves the server; the hook gives live reactive role
      checks so a demoted admin loses the nav link mid-session without a
      reload.
    - Role helpers (isSystemAdmin / isCompanyAdmin) centralised in
      lib/roles.ts so the whole codebase points at one source of truth and
      nobody inlines `u?.role === 'admin'` at call sites.
    - Signature canvas is HTML5 Canvas + Pointer Events (not a third-party
      lib). DPR-aware resize, quadratic-curve smoothing, 2.5px black ink
      with rounded caps/joins to match iOS SignatureCaptureView. Existing
      signature loads as a background image so re-drawing flattens strokes
      on top into the exported PNG — matches iOS merge semantics.
    - Two-step save (upload blob FIRST, THEN PUT profiles array) so a failed
      upload never leaves the profile pointing at a non-existent S3 key.
    - Backend signature GET route added because the upload returns an S3
      key with no corresponding auth'd read endpoint — browsers can't
      attach a bearer token to a bare S3 URL. Mirrors the photo-download
      pattern in src/routes/photos.js including the basename path-traversal
      guard and `Cache-Control: private, max-age=300`.
    - Default-inspector mutex applied client-side before PUT so backend
      sees canonical single-default state (matches iOS behaviour where the
      list is a full array, not individual PATCH endpoints).
    - First-ever profile auto-defaults to is_default: true so a freshly
      onboarded company always has a default for cert generation.
    - Equipment section is collapsible and auto-expands only if any slot
      is populated — keeps the initial form clean for profiles without
      instruments (most admins don't need them).
    
    CONTEXT
    - Branch: web-rebuild. Follows Phase 5d (LiveFillView).
    - Scope: 6a only. 6b (company settings + company-admin dashboard) and
      6c (system-admin user CRUD) are intentionally stubbed as disabled
      "Coming in Phase 6b/6c" cards on the hub per the handoff's
      stop-the-line rule.
    - Verification: tsc --noEmit clean, ESLint clean at baseline
      (6 pre-existing warnings on unrelated files). Next build error on
      /login confirmed pre-existing on baseline commit cccd548 (not a
...
```
 CLAUDE.md                                         |   1 +
 src/routes/settings.js                            |  47 +++
 web/src/app/settings/layout.tsx                   |  10 +
 web/src/app/settings/page.tsx                     | 213 +++++++++++
 web/src/app/settings/staff/[inspectorId]/page.tsx | 418 ++++++++++++++++++++++
 web/src/app/settings/staff/page.tsx               | 312 ++++++++++++++++
 web/src/components/settings/signature-canvas.tsx  | 297 +++++++++++++++
 web/src/lib/api-client.ts                         |  82 +++++
 web/src/lib/roles.ts                              |  30 ++
 web/src/lib/types.ts                              |  72 ++++
 web/src/lib/use-current-user.ts                   |  52 +++
 web/src/middleware.ts                             |  33 +-
 12 files changed, 1558 insertions(+), 9 deletions(-)
```
