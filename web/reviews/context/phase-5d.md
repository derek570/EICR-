...
```
commit cccd5487c83ae98f678af6b6c59839f1a25b471f
Author:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
AuthorDate: Fri Apr 17 17:22:45 2026 +0100
Commit:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
CommitDate: Fri Apr 17 17:22:45 2026 +0100

    feat(web): Phase 5d LiveFillView
    
    While recording, swap the active tab's content for a full-form live
    dashboard that flashes brand-blue every time Sonnet fills a field,
    fading back to transparent over ~2s. Ports iOS LiveFillView.swift so
    the web client matches iOS parity — inspectors no longer have to tab-
    hunt mid-session to verify what's been captured, which was breaking
    the "speak hands-free, verify at end" workflow.
    
    Why this design:
    
      * Per-field recency is stored in a module-level LiveFillStore
        (Map<fieldKey, timestamp>), exposed via useSyncExternalStore with
        a per-field `useIsFieldRecent(key)` selector. One markUpdated call
        only re-renders the <LiveField> leaves whose boolean actually
        flipped — a context + provider would have re-rendered every leaf.
      * changedKeys is computed by DIFFING pre-patch job vs patch inside
        applyExtractionToJob, not by reading the extractor's readings
        array. Diffing stays correct even if the extractor grows new
        fields or the routing table drifts — important because the
        server-side Sonnet prompt evolves independently of the client.
      * Key convention locked in: `section.field` for scalars,
        `circuit.{id}.field` for circuit cells, `circuit.{id}` for a
        newly-created row (flashes the whole row), `observation.{id}` for
        new observations. Every producer must use these so <LiveField>
        subscriptions land on the right leaves.
      * <LiveField> is CSS-only — toggling `data-recent="true|false"` re-
        triggers `transition: background-color 2s ease-out` automatically.
        No keyframes, no per-field JS timer; one bundled 3050ms cleanup
        sweep in the store handles pruning for every key at once.
        `@media (prefers-reduced-motion: reduce)` shortens to 200ms so the
        signal is still visible without the slow animation.
      * Mount site is Option A from the handoff — a new <JobBody> reads
        useRecording().state inside RecordingProvider and swaps
        {children} for <LiveFillView> whenever state is
        active/dozing/sleeping. Matches iOS exactly and avoids z-index
        juggling: TranscriptBar + FloatingActionBar + RecordingOverlay
        stay mounted at their usual positions.
      * Empty-value cells render an em-dash so the layout stays stable as
        fields populate — preserves spatial memory (iOS parity point #4
        in the handoff). Section entrance stagger via nth-child CSS
        animation keeps the initial render calm.
      * Observations gated on EICR (EIC has none); Extent gated on EIC
        (EICR has no extent card). Mirrors iOS.
    
    Scope exclusions (handoff §"Scope exclusions"): typing-per-character
    animation, CCU slot-correction grid, landscape-specific compact
    layout, per-observation fade-in. Defer until inspectors ask — the
    background flash alone conveys the "just filled" signal.
    
    Verify: tsc --noEmit clean, lint unchanged (0 errors, 6 pre-existing
    warnings). Phase 5 closed; next up is Phase 6 Admin/Settings parity.
...
```
 CLAUDE.md                                       |   1 +
 web/src/app/globals.css                         |  64 ++++
 web/src/app/job/[id]/layout.tsx                 |  24 +-
 web/src/components/live-fill/live-field.tsx     | 119 +++++++
 web/src/components/live-fill/live-fill-view.tsx | 421 ++++++++++++++++++++++++
 web/src/lib/recording-context.tsx               |  28 +-
 web/src/lib/recording/apply-extraction.ts       | 109 +++++-
 web/src/lib/recording/live-fill-state.ts        | 216 ++++++++++++
 8 files changed, 968 insertions(+), 14 deletions(-)
```

## Handoff doc: PHASE_5D_HANDOFF.md
