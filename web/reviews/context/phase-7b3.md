...
```
commit a85487f6b4ec2c09b3852288a447580223a184de
Author:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
AuthorDate: Fri Apr 17 22:25:39 2026 +0100
Commit:     Derek Beckley <derekbeckley@Dereks-Mac-mini.broadband>
CommitDate: Fri Apr 17 22:25:39 2026 +0100

    feat(web): Phase 7b — AppShell offline indicator
    
    Adds an amber "Offline" pill to the AppShell header right-cluster
    that appears whenever the browser reports navigator.onLine === false.
    Pairs with the IDB cache (2d3527f) so the inspector knows WHY the
    dashboard/job looks the same as last time — the SWR cache may be
    showing stale data and the pill is how they know connectivity is
    the reason.
    
    Closes PHASE_7A_HANDOFF.md §"Scope exclusions":
      "No offline indicator in AppShell — the navigator.onLine +
       retry-on-online UX is 7b."
    
    Files:
      - web/src/lib/pwa/use-online-status.ts — minimal hook wrapping
        navigator.onLine + online/offline window events. Returns a
        plain boolean. SSR-safe: defaults to true (optimistic) on
        server + first client render; the useEffect flips to the
        real navigator.onLine on mount. Defaulting to false would
        flash the pill on every cold render for online users.
        Plain useState, not useSyncExternalStore — this is a binary
        that changes at most a handful of times per session, the
        subscription cost isn't warranted.
    
      - web/src/components/pwa/offline-indicator.tsx — the pill:
        rounded border + 15%-opacity fill in --color-status-processing
        (amber #ff9f0a), WifiOff lucide icon, "Offline" text on sm+
        / icon-only below. Amber NOT red: red reads as "something
        broke" (destructive); offline-with-cache is degraded, not
        broken — red would be panic-inducing for a state the SWR
        layer handles gracefully. Icon-only below sm because the
        mobile 56px header already packs Logo + name + install +
        sign-out; forcing the label at 320px (iPhone SE) would push
        Sign-out off-screen. title + aria-label always carry the
        full "Showing cached data; changes will not sync until your
        connection returns." string so hover + screen readers aren't
        starved by the responsive hide. role=status + aria-live
        polite (NOT assertive — informational, not an action the
        user must take right now). Returns null when online for
        zero DOM footprint in the common case.
    
      - web/src/components/layout/app-shell.tsx — mounts the pill as
        the FIRST item in the header right-cluster so the eye lands
        on connection state before name/install/sign-out. The
        existing `flex items-center gap-3` absorbs appear/disappear
        without shifting neighbours.
    
    No "back online" confirmation toast:
      Serwist's reloadOnOnline: true (set in next.config.ts since 7a)
      already triggers window.location.reload() when the browser
      fires `online` after being offline. By the time any toast
      would render, the page is reloading — the toast would flash
      for a frame and disappear. The visible pill disappearing IS
      the confirmation.
    
    navigator.onLine caveat (documented in both files):
      `true` just means the device has a network interface, NOT that
      requests succeed. Captive-portal wifi, DNS hijack, and ISP
      blocks all look "online" to the browser. That's why this hook
      drives only the visible pill — actual retry logic is driven
      by failed fetches in the 2d3527f SWR paths, not onLine
      transitions.
    
    Scope still deferred:
      - iOS "Add to Home Screen" hint on /settings (next 7b commit —
        Safari never fires beforeinstallprompt so a dismissible
        banner is the planned accommodation)
      - 7c outbox / mutation queue
      - 7d offline job-edit UI polish
    
    Verification:
      - npm run typecheck   : clean
      - npm run lint        : 0 errors, 6 warnings (baseline unchanged)
      - npm run build       : succeeds
    
    Manual DevTools check (Network → Offline toggle):
      - toggle off → amber "Offline" pill appears in header
      - toggle on  → pill disappears + reloadOnOnline reloads
      - 375px / 390px viewports: icon-only keeps Sign-out visible
      - sm+ (640px+): "Offline" label surfaces
...
```
 ...
 web/src/components/pwa/offline-indicator.tsx | 62 ++++++++++++++++++++++++++++
 web/src/lib/pwa/use-online-status.ts         | 54 ++++++++++++++++++++++++
