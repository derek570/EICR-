...
| recording/readback-dedup-rekey | iOS confirmation dedupe key = field+circuit+board+text-hash (2026-06-18) + circuits-list in key (`8b51418`) | `confirmation-dedupe-key.ts` — literal port (djb2 UInt64 BigInt, three shapes), wired at the D6 confirmation loop; `Confirmation` type gains `circuits`/`board_id` | match | 2026-07-02 | WS3 | SHIPPED 2026-07-02 (item 2). Hash vectors generated from the backend mirror `src/extraction/ios-dedupe-key.js` and pinned. Never-clear bare-"no": verified — web has NO client field-clearing path (clears are server-driven `field_clears` only) and the gate forwards bare negations for the backend read-back window to resolve. |
...
| circuits/lim-sentinel-display | iOS IR fields accept/display the `LIM` sentinel (word-boundaried; 2026-06-16 wave made it first-class server-side) | AUDITED COMPLETE 2026-07-02: regex byte-identical to iOS (IR L-E/L-L + supply/main-switch), apply stores raw strings, grid cells type=text | match | 2026-07-02 | WS3 | Item 6 audit outcome: zero runtime gaps; 9 regression tests added (`lim-sentinel.test.ts`). Known SHARED iOS/web quirk documented in tests: digit-prefixed single utterance hits the numeric pattern first (Sonnet tier corrects). WS7 keyboard-accessory LIM button wires into this path. |
