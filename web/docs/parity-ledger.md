...
| `DashboardView.swift:L708-L724` `createAndNavigate` with auto-apply-defaults + preset picker sheet | `dashboard/page.tsx` simple create; Circuits tab Apply Defaults on demand | partial | 9 | Phase 6 ships the Defaults editor + user-scoped storage so Apply Defaults (Circuits tab) now pulls from saved values. Auto-apply-on-create + preset picker sheet deferred to Phase 9 — inspectors prefer explicitly triggering the fill once CCU photo or document extraction has added circuits. |
...
| `DashboardView.swift:L188-L189` pull-to-refresh `.refreshable` | MISSING | partial | 3 | Phase 3 decision: skipped — SWR IDB cache already serves the offline/stale path. Re-evaluate in Phase 9 if inspectors request it. |
...
| `Views/Dashboard/InspectorModal.swift` (quick inspector picker from dashboard) | MISSING | missing | 9 | Quick-pick dashboard modal deferred — inspectors set a default inspector in Staff settings which auto-populates on new jobs; on-demand inspector switch happens from the Staff tab inside the job. |
...
| `CircuitsTab.swift:L272-L353` PendingExtractionQueue banner + thumbnails + Retry All + auto-retry on network restore | MISSING | missing | 7 | Scoped-out in Phase 7. Web's mutation outbox (Phase 7c) already replays failed PATCHes; replicating the photo-upload queue requires IDB blob storage + a separate replay worker because `/api/analyze-ccu` isn't a standard PATCH. Punt to Phase 10 unless inspector feedback surfaces demand. |
| `CircuitsTab.swift:L486-L491` Auto-retry on `NetworkMonitor.shared.isConnected` flip | MISSING | missing | 7 | Paired with the pending queue row above — same scoping decision. |
...
| `CircuitsTab.swift:L278-L295` Pending extractions section — thumbnails + Retry All | MISSING | missing | 7 | See CCU extraction flow table above — scoped out of Phase 7; tracked under the PendingExtractionQueue row. |
...
| `recording-chrome.tsx` Apply button `disabledReason="Apply-last-snapshot is iOS-only for now."` | covered above | missing | 8 | Still disabled — depends on a defaults snapshot repo that isn't in scope for the recording-polish phase. Tracked as a follow-up under the Defaults parity row. |
...
| `SettingsHubView.swift:L209-L213` Terms & Legal → `TermsAcceptanceView` | MISSING | missing | 9 | Deferred: legal copy ports can land in Phase 9 polish; no current compliance blocker on web (handled at signup flow). |
