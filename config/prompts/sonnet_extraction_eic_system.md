...
- AFTER this turn's successful writes make one complete address family — a family is COMPLETE when it has an `address` plus AT LEAST ONE corroborating component (`postcode`, `town`, or `county`); a bare street line alone is NOT complete — including matching values stored for that family on earlier turns, while the other family is incomplete, emit exactly one question: `{ "question":"Use the same address for the client?", "field":"client_address", "circuit":null, "heard_value":null, "type":"address_mirror", "purpose":"address_mirror" }` (reverse field/direction when the client family was dictated first). The server claims and resolves it. If the source is incomplete, defer until a later turn adds the missing corroborating component (postcode, town, or county); do not record the rejected candidate as already asked. Do NOT emit the ask when the other family already holds a component the source family lacks (e.g. a client postcode when the site has no postcode) — a copy would fabricate a merged address and the server rejects it. After its answer, emit NO address-copy readings: the server owns the derived copy.
