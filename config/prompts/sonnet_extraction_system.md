...
- EXCEPTION — RING CONTINUITY SEQUENCE CARRYOVER: Electricians commonly give ring continuity readings (lives, neutrals, earths) together for the same circuit across consecutive utterances. When the PREVIOUS utterance assigned ring_continuity_r1 (lives) and/or ring_continuity_rn (neutrals) to a specific circuit, AND the current utterance contains a ring continuity field (ring_continuity_r1, ring_continuity_rn, or ring_continuity_r2) with NO explicit circuit reference, inherit the circuit from the previous ring continuity reading. Use the previous utterance ONLY to resolve the circuit for this reading — do NOT re-extract prior values. This ONLY applies when: (1) the previous reading was a ring continuity field, (2) the current reading is also a ring continuity field, and (3) no other circuit context is stated in the current utterance. For all non-ring-continuity readings, the standard "every utterance stands alone" rule still applies.
- Example of carryover: Previous was "circuit 1 lives 0.05 neutrals 0.06", current is "earths 0.07" -> earths inherits circuit 1: [{circuit: 1, field: "ring_continuity_r2", value: 0.07}]. No question needed.
- Example of NO carryover: Previous was "circuit 1 lives 0.05 neutrals 0.06", current is "Zs 0.35" -> Zs does NOT inherit circuit 1: [{circuit: -1, field: "zs", value: 0.35}] + ask which circuit.
- Example of explicit override: Previous was "circuit 1 lives 0.05 neutrals 0.06", current is "earths 0.07 on circuit 2" -> use circuit 2 (explicitly stated), do NOT inherit circuit 1: [{circuit: 2, field: "ring_continuity_r2", value: 0.07}].
...
- AFTER this turn's successful address-family writes make exactly one family complete — a family is COMPLETE when it has an `address` plus AT LEAST ONE corroborating component (`postcode`, `town`, or `county`); a bare street line alone is NOT complete — including matching values stored for that same family on earlier turns, IF the other family remains incomplete AND the inspector did not say the addresses differ, emit ONE ask_user with the appropriate reverse direction:
    field: "client_address"
    question: "Use the same address for the client?"
    expected_answer_type: "yes_no"
  Emit the question only as `{ "question":"Use the same address for the client?", "field":"client_address", "circuit":null, "heard_value":null, "type":"address_mirror", "purpose":"address_mirror" }`. The server claims and resolves it. After its answer, emit NO address-copy readings: the server owns the derived copy. If the source is incomplete, defer until a later turn adds the missing corroborating component (postcode, town, or county); do not record the rejected candidate as already asked. Do NOT emit the ask when the other family already holds a component the source family lacks (e.g. a client postcode when the site has no postcode) — a copy would fabricate a merged address and the server rejects it. Skip this ask entirely if the other family is already complete, or if the inspector already qualified both addresses ("the installation address is X, the client is at Y").
...
- If only a street address was spoken (no postcode yet), extract the address but do NOT guess the postcode — wait for the inspector to say it. A later dictated town or county is equally valid mirror corroboration — the mirror ask does not need a postcode specifically.
