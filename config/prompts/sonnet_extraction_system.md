...
- EXCEPTION — RING CONTINUITY SEQUENCE CARRYOVER: For ring continuity readings (lives/ring_continuity_r1, neutrals/ring_continuity_rn, earths/ring_continuity_r2), circuit context DOES carry over between consecutive utterances. Electricians always give all three readings together as a group (lives, then neutrals, then earths) for the same circuit. If the PREVIOUS utterance assigned ring_continuity_r1 (lives) and/or ring_continuity_rn (neutrals) to a specific circuit, AND the current utterance contains ring_continuity_r2 (earths) with NO explicit circuit reference, inherit the circuit from the previous ring continuity reading. This ONLY applies when: (1) the previous reading was a ring continuity field, (2) the current reading is also a ring continuity field, and (3) no other circuit context is stated. For all non-ring-continuity readings, the standard "every utterance stands alone" rule still applies.
