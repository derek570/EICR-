...
| One job permanently unsyncable for a WEEK (`job_1778443465217`); client retried identical PUT every 30s 2026-06-05→06-12; all inspector edits silently lost | PUT board-hierarchy validator REJECTED (400) any invalid hierarchy — a dangling `feed_circuit_ref` made every save fail | Rearchitected: PUT path REPAIRS deterministically (clear dangling pointers, demote duplicate mains), reports `hierarchy_repairs`; strict validation stays only on interactive `add_board`. `45c4e8aa` (2026-06-12), `repairBoardHierarchy` in `src/extraction/board-hierarchy-validator.js`. **2026-09-18 sequel:** the body echo itself wedged iOS (its `[String: Bool]` decode of the PUT response threw on the extra keys → job dirty forever, never fetched, nine re-pushes in three minutes on job_1789724466336); the body is now always exactly `{success:true}` and the repairs moved to the `X-Hierarchy-Repairs` header | SETTLED DOCTRINE: persistence-path validation repairs, never rejects, and the PUT body shape is FROZEN at `{success:true}` — put diagnostics in headers. Do not re-add a reject gate to PUT |
