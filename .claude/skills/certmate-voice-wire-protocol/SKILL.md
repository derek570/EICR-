...
| `job_state_update` — installation address family (Plan E, feedback id 125) | `installation` (iOS) / `installationDetails` / `installation_details` (web, snake_case internally but NORMALISED to this shape at the producer) `= {address, postcode, town, county, clientAddress, clientPostcode, clientTown, clientCounty}` | The FROZEN wire contract, iOS field-naming shape on both clients. Backend `selectInstallationContainer` (`eicr-extraction-session.js`) merges all three bucket-key spellings per field, iOS-first precedence. SEED (session start, empty target) writes all 8 unconditionally; MID-SESSION MERGE is fill-EMPTY-ONLY (the 8 keys are deliberately excluded from `FACT_FIELDS` — a stale client push can fill a gap but never clobbers a fresher Sonnet-dictated correction). `job-state-frame.js`'s `JOB_STATE_FIELDS` allowlist must list all three bucket-key spellings or an installation-only frame is dropped as envelope noise before the merge ever runs. |
...
| `field_corrected` | `{circuit, field, previous_value\|null, reason\|null, board_id?, utterance_id?}` | Stage-6 `clear_reading`; emitted per-event AFTER the extraction envelope. `board_id` (A1a/A1b) marks a board-scope clear (`circuit:null`). `utterance_id` (A02D, 2026-09-09) = the client-minted id of the utterance that fed the turn, the same value the extraction frame and VCR echo — additive, absent when the result has none; the client maps it to that final's sequence for its clear/replacement cutoff. |
