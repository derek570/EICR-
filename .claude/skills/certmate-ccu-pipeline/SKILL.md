...
2. **One VLM call** with `MODERN_PROMPT` (:149) or `REWIREABLE_PROMPT` (:223). Prompt contract: one entry per module slot, strict left-to-right; 2-pole devices = two identical entries; blanks are entries (`device_kind:"blank"`) — including bare-rail EMPTY ways at either END of the rail, keyed off the numbered label strip (added 2026-09-18 after a 10-way Wylex came back as 9); an explicit toggle-counting procedure for runs of identical MCBs, with the rule *"if unsure between N and N+1 … prefer N+1"* (a phantom slot is recoverable, a missed slot is not); labels returned as a SEPARATE `labels[]` array with `position_x` 0-1 — the prompt explicitly forbids the model from assigning labels to devices itself.
...
Order matters:
1. **`applyRcdTypeLookup`** (`src/extraction/rcd-type-lookup.js` + `config/rcd-type-lookup.json`) — (manufacturer, model) table. Confidence policy for MODEL-level entries: `high` overrides every RCD-protected circuit's `rcd_type` (the sub-mm waveform glyph read is less reliable than a known datasheet); `medium` = default, a ≥0.95-confidence slot read wins; `low` = fill nulls only. **A `manufacturer_default` hit is fill-null ONLY, whatever its confidence (since 2026-09-18):** single-shot pins every slot at 0.92/0.65 so the ≥0.95 escape was unreachable and the Wylex default (A) overwrote a correct per-slot AC read on a field certificate. Non-hits write a fire-and-forget "pending sighting" to S3 (`writeRcdPendingEntry`) so the table can be grown by a review CLI.
