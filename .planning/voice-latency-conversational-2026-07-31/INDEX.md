...
| — | [08D — terminal-round release](plan-08d-terminal-round-release.md) | **CLOSED docs-only 2026-08-11** (session `8B9B2BDD`, `eicr-backend:393`). The former 08B §2.0, extracted at round 2 on 2026-08-10. **Still the wave's largest measured lever** — every turn ends with a no-tool `end_turn` round costing p50 1710–1712 ms / **24–34 % of perceived latency depending on denominator**, emitting a median of **4** tokens that are never spoken (`bundler_only` ×28), never enter history (`eicr-extraction-session.js:2874`), and gating audio already synthesised and parked (audio-ready precedes loop completion **18/18**, median lead 1596 ms, floor 843 ms). Carries the four (now five, counting 08D's own reviewed attempt) binding precedents of the reverted/failed attempts. **Ships no runtime mechanism** — the current transport has no trustworthy pre-completion no-tool signal; the shrink/eliminate-round lever transfers to **08C** in full, including the replication gate and acceptance preconditions. |
...
| [08D — terminal-round release](plan-08d-terminal-round-release.md) | Release the inspector before the loop returns, instead of waiting on the no-tool `end_turn` round (the former 08B §2.0) | **Tier B, CLOSED docs-only 2026-08-11** — session `8B9B2BDD`/`:393` discharged the park; ships no runtime early-release mechanism (transport has no trustworthy pre-completion signal, five mechanism attempts now dead). Largest measured lever (24–34 % of perceived latency depending on denominator); the lever itself transfers to 08C |
