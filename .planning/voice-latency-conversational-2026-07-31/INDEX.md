...
2. **Execution order is now `Tier A → 08A → 08B → 06`**, with 02 demoted (see below) and
   ...
   and 08D awaited has since run** (session `8B9B2BDD`, `eicr-backend:392`, 2026-08-11): **08D —
   ...
   the transport level; **08C split into 08C-A (per-round cost — SHIPPED 2026-08-17, PR #189: NO
   ARM WINS, keep `recent_3`) and 08C-B (terminal-round lever — CLOSED 2026-08-17, closure
   CONFIRMED BY MEASUREMENT)**, 08C-B carrying 08D's shrink/eliminate-round lever plus its
   transferred replication gate and acceptance preconditions. 08C-B's feasibility gate
   resolved to NO VIABLE MECHANISM across five axes (tier already priority, prefix cache
   engaged, connection warm, predicted outputs unavailable; 90-rep bench replicated the
   terminal-round floor at p50 1,266 ms across 1,332 turns). The Codex review's one missed
   candidate — **Responses-WS / `previous_response_id` state chaining** — was probed the same
   day and disproved (~13 ms p50 for a 205× smaller payload; see the plan's closure section
   and `evidence-08c-b-prid-probe-2026-08-17.json`). **The 08-series is complete.**
...
|---|---|---|
| 1 | [08A — see inside a model round](plan-08a-stage6-round-instrumentation.md) | **SHIPPED 2026-08-10.** Telemetry settled where the latency is: summed round `stream_ms` ÷ perceived latency is **0.91–0.97** — model rounds are ~90 %+ of the loop, TTS is 3–9 %. But we could not see *inside* a round. Three additive fields (`reasoning_tokens`, `first_tool_use_ns`, `blocking_ask_user_dispatched`) plus a timing row on all seven post-completion exits, zero behaviour change. Live on `eicr-backend:388`. **That ordinary field session has now run** (`8B9B2BDD`, `eicr-backend:392`, 2026-08-11) — it unparked 08C (pending its own checklist confirmation) and discharged 08D's park condition; 08D has since closed docs-only. |
...
| — | [08D — terminal-round release](plan-08d-terminal-round-release.md) | **CLOSED docs-only 2026-08-11** (session `8B9B2BDD`, `eicr-backend:392`). The former 08B §2.0, extracted at round 2 on 2026-08-10. **Still the wave's largest measured lever** — every turn ends with a no-tool `end_turn` round costing p50 1710–1712 ms: **24–28.3 % of whole-turn perceived latency** in the original 28-turn corpus, **34.0 % of round-stream-time only** in the new 12-turn corpus (non-comparable denominators — the honest headline is the character change to round-trip floor, not a bigger percentage), emitting a median of **4** tokens that are never spoken (`bundler_only` ×28), never enter history (`eicr-extraction-session.js:2874`), and gating audio already synthesised and parked (audio-ready precedes loop completion **18/18**, median lead 1596 ms, floor 843 ms). Every proposed mechanism is now dead — the ones that preceded this plan's own refine cycle, plus the "safe door" independently disproven within it. **Ships no runtime mechanism** — the current transport has no trustworthy pre-completion no-tool signal; the shrink/eliminate-round lever transfers to **08C** in full, including the replication gate and acceptance preconditions. |
| — | [08C-A](plan-08c-a-per-round-cost.md) / [08C-B](plan-08c-b-terminal-round.md) — per-round cost levers | **Split 2026-08-11** (session `8B9B2BDD`, `eicr-backend:392`; the three-item checklist — revision `:388+`, `api_transport` split, `round_idx` keying — was independently confirmed) into two chained chunks. **08C-A (config-lever probes + the §1.4/§2.1 prefix/snapshot pair): SHIPPED 2026-08-17 (PR #189) — NO ARM WINS, keep `recent_3`** (ascending +0.9%, window_6 +3.7%, both under the +10% bar; eviction duplicate-write causality REFUTED); benchmark harness + operator tooling delivered dark, `SNAPSHOT_RECENT_CIRCUITS` unchanged. **08C-B (08D-inherited terminal-round shrink/eliminate lever + its replication gate + four acceptance preconditions): CLOSED 2026-08-17, confirmed by measurement.** The feasibility gate (deliverable 1) resolved to NO VIABLE MECHANISM across five axes — tier already priority, prefix cache engaged (1,331/1,332 bench terminal rows read a ~35.7–35.8k prefix), connection warm, predicted outputs structurally unavailable (Chat-Completions-only, unsupported on gpt-5.x, decode-side while the terminal round decodes 4 tokens), and the Codex hold's one missed candidate — **Responses-WS / `previous_response_id` state chaining — probed the same day and DISPROVED** (n=12/arm interleaved A/B on `gpt-5.6-luna`: p50 first-text 902 → 889 ms for a 146 KB → 713 B payload; the chained call still runs the full cached prefill server-side; `evidence-08c-b-prid-probe-2026-08-17.json`). The 90-rep bench corroborates a stable floor: 1,163/1,332 turns (87.3 %) zero-reasoning terminal at p50 **1,266 ms** vs the corpus's 1,267 ms. Audio-ready telemetry NOT shipped (does not ride the failed gate); deliverable-2 CONTRACT preserved. Side-finding queued to Derek: the org retains `/v1/responses` responses 30 days by DEFAULT (`store` defaults true; adapter sends no param) — `store:false` is a separate privacy decision with zero latency impact. See the closure section in [plan-08c-b-terminal-round.md](plan-08c-b-terminal-round.md). |
...
| [08A — see inside a model round](plan-08a-stage6-round-instrumentation.md) | Additive round telemetry: reasoning tokens, first-`tool_use` stamp, `ask_user` wait marking, timing on all seven post-completion paths | **SHIPPED 2026-08-10** — zero behaviour change; one ordinary field session on the deployed build has now run (`8B9B2BDD`, `:392`, 2026-08-11) and yielded 08C's data |
...
| [08C-A](plan-08c-a-per-round-cost.md) / [08C-B](plan-08c-b-terminal-round.md) — per-round cost levers | Cut the cost of each round: reasoning effort, round-1 model, prompt snapshot size; 08C-B owns 08D's shrink/eliminate-terminal-round lever | **Tier B, split 2026-08-11** — field session `8B9B2BDD`/`:392` discharged the park. **08C-A SHIPPED 2026-08-17 (PR #189): NO ARM WINS, keep `recent_3`.** **08C-B CLOSED 2026-08-17, confirmed by measurement** — NO VIABLE MECHANISM (terminal-round floor replicated at p50 1,266 ms over 1,332 bench turns); the Responses-WS/`previous_response_id` chaining candidate was probed same-day and disproved (~13 ms p50). 08-series complete |
| [08D — terminal-round release](plan-08d-terminal-round-release.md) | Release the inspector before the loop returns, instead of waiting on the no-tool `end_turn` round (the former 08B §2.0) | **Tier B, CLOSED docs-only 2026-08-11** — session `8B9B2BDD`/`:392` discharged the park; ships no runtime early-release mechanism (transport has no trustworthy pre-completion signal; every proposed mechanism now dead). Largest measured lever — 24–28.3 % of whole-turn latency (28-turn corpus) / 34.0 % of round-stream-time only (12-turn corpus, non-comparable denominators); the lever itself transfers to 08C |
