...
2. **Execution order is now `Tier A → 08A → 08B → 06`**, with 02 demoted (see below) and
   ...
   ARM WINS, keep `recent_3`) and 08C-B (terminal-round lever — CLOSED docs-only 2026-08-17)**,
   08C-B carrying 08D's shrink/eliminate-round lever plus its transferred replication gate and
   acceptance preconditions. 08C-B's feasibility gate resolved to NO VIABLE MECHANISM — all four
   content-independent transport axes (tier, prefix cache, connection, predicted outputs) are
   exhausted or unavailable, and the 90-rep bench replicated the terminal-round floor at p50
   1,266 ms across 1,332 turns. With 08C-B closed, the 08-series is COMPLETE.
...
| — | [08C-A](plan-08c-a-per-round-cost.md) / [08C-B](plan-08c-b-terminal-round.md) — per-round cost levers | **Split 2026-08-11** (session `8B9B2BDD`, `eicr-backend:392`; the three-item checklist — revision `:388+`, `api_transport` split, `round_idx` keying — was independently confirmed) into two chained chunks. **08C-A (config-lever probes + the §1.4/§2.1 prefix/snapshot pair): SHIPPED 2026-08-17 (PR #189) — NO ARM WINS, keep `recent_3`** (ascending +0.9%, window_6 +3.7%, both under the +10% bar; eviction duplicate-write causality REFUTED); benchmark harness + operator tooling delivered dark, `SNAPSHOT_RECENT_CIRCUITS` unchanged. **08C-B (08D-inherited terminal-round shrink/eliminate lever + its replication gate + four acceptance preconditions): CLOSED docs-only 2026-08-17** — its feasibility-first gate (deliverable 1) resolved to NO VIABLE MECHANISM: all four content-independent transport axes are exhausted (tier already priority, prefix cache already free, connection already warm) or structurally unavailable (predicted outputs: Chat-Completions-only, unsupported on gpt-5.x reasoning models, and decode-side anyway — the terminal round decodes 4 tokens). The 90-rep bench corroborates a stable floor: 1,163/1,332 turns (87.3 %) zero-reasoning terminal at p50 **1,266 ms** vs the corpus's 1,267 ms. Audio-ready telemetry deliberately NOT shipped (does not ride a failed gate); deliverable-2 CONTRACT preserved for any future separately-argued follow-up. See the closure section in [plan-08c-b-terminal-round.md](plan-08c-b-terminal-round.md). |
