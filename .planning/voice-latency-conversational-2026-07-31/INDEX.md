...
2. **Execution order is now `Tier A → 08A → 08B → 06`**, with 02 demoted (see below) and
   ...
   ARM WINS, keep `recent_3`) and 08C-B (terminal-round lever — closure PROPOSED 2026-08-17,
   HELD in Codex review)**, 08C-B carrying 08D's shrink/eliminate-round lever plus its
   transferred replication gate and acceptance preconditions. 08C-B's feasibility gate was
   argued to NO VIABLE MECHANISM (tier already priority, prefix cache engaged, connection
   plausibly warm, predicted outputs unavailable; 90-rep bench replicated the terminal-round
   floor at p50 1,266 ms across 1,332 turns) — but the review found the inventory omits the
   **Responses-WS / `previous_response_id` state-chaining candidate**, so the closure is HELD
   pending Derek's decision (see the plan's closure banner). The 08-series is NOT yet complete.
...
| — | [08C-A](plan-08c-a-per-round-cost.md) / [08C-B](plan-08c-b-terminal-round.md) — per-round cost levers | **Split 2026-08-11** (session `8B9B2BDD`, `eicr-backend:392`; the three-item checklist — revision `:388+`, `api_transport` split, `round_idx` keying — was independently confirmed) into two chained chunks. **08C-A (config-lever probes + the §1.4/§2.1 prefix/snapshot pair): SHIPPED 2026-08-17 (PR #189) — NO ARM WINS, keep `recent_3`** (ascending +0.9%, window_6 +3.7%, both under the +10% bar; eviction duplicate-write causality REFUTED); benchmark harness + operator tooling delivered dark, `SNAPSHOT_RECENT_CIRCUITS` unchanged. **08C-B (08D-inherited terminal-round shrink/eliminate lever + its replication gate + four acceptance preconditions): closure PROPOSED 2026-08-17, HELD in Codex review.** The feasibility gate (deliverable 1) was argued to NO VIABLE MECHANISM — tier already priority, prefix cache engaged (1,331/1,332 bench terminal rows read a ~35.7–35.8k prefix), connection plausibly warm (socket reuse unmeasured), predicted outputs structurally unavailable (Chat-Completions-only, unsupported on gpt-5.x, decode-side while the terminal round decodes 4 tokens). The 90-rep bench corroborates a stable floor: 1,163/1,332 turns (87.3 %) zero-reasoning terminal at p50 **1,266 ms** vs the corpus's 1,267 ms. **HELD because the inventory omits Responses-WS / `previous_response_id` state chaining** — documented low-latency continuation for exactly this loop shape, undisprovable without measurement; Derek decides: evaluate it (re-opens deliverables 2–4 as a dark build) or accept the closure. Audio-ready telemetry NOT shipped either way (does not ride the provisionally-failed, now held, gate); deliverable-2 CONTRACT preserved. See the closure banner in [plan-08c-b-terminal-round.md](plan-08c-b-terminal-round.md). |
