...
2. **Execution order is now `Tier A → 08A → 08B → 06`**, with 02 demoted (see below) and
   ...
   mechanism while §2.1 drew zero. 08B is the reviewable remainder; **08C and 08D are both parked
   on the same one field session** on `eicr-backend:388`+ and take whatever slot the data arrives
   in. Note 08D holds the wave's largest single lever (24 % of perceived latency), so "parked" here
   means deferred-and-hard, not deprioritised.
...
| — | [08D — terminal-round release](plan-08d-terminal-round-release.md) | **PARKED — blocked on the same field session on `eicr-backend:388`+ as 08C.** The former 08B §2.0, extracted at round 2 on 2026-08-10. **Still the wave's largest measured lever** — every turn ends with a no-tool `end_turn` round costing p50 1712 ms / **24 % of perceived latency**, emitting a median of **4** tokens that are never spoken (`bundler_only` ×28), never enter history (`eicr-extraction-session.js:2874`), and gating audio already synthesised and parked (audio-ready precedes loop completion **18/18**, median lead 1596 ms, floor 843 ms). Carries the four binding precedents of the reverted attempts. **Removes no round** — it changes when the human is released from the round that always runs. |
...
| [08D — terminal-round release](plan-08d-terminal-round-release.md) | Release the inspector before the loop returns, instead of waiting on the no-tool `end_turn` round (the former 08B §2.0) | **Tier B, PARKED** — extracted at round 2 after six BLOCKERs; gated on the same `:388` field session as 08C. Largest measured lever (24 % of perceived latency) and the highest-risk: four reverted precedents |
