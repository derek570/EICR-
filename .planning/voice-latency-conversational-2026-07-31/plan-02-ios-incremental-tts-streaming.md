...
Status: **TIER C — DEMOTED 2026-08-10, RATIFIED by Derek 2026-08-16.** Plan 07 measured the Loaded Barrel hit rate at 78%, on which incremental playback buys nothing (the audio is already parked ~1.6s before the model loop finishes); on the 22% miss it buys the 147ms synthesis tail Plan 03 measured. Amortised: `147ms × 22% ≈ 32ms` against a 6.4s p50 — about 0.5%. The wave reserved an explicit re-argue moment after 08A that resolved early without that data (see `~/obsidian-vault/active/ep-digest.md`'s 2026-08-10 entry, now Resolved); Derek's ruling on re-review: the arithmetic doesn't justify the iOS build cost (capability handshake + exactly-once playback ACK as shared concurrent state, full dual-reviewer `/rp` loop). Stays unbuilt unless a non-latency reason (perceived responsiveness, streaming-first groundwork) reopens it — that argument would need its own `/rp`, not a plan-doc edit. Fold the iOS TTS watchdog fix into this work if it has not already shipped as a Tier A item — both live in `AlertManager.swift`'s playback path.
