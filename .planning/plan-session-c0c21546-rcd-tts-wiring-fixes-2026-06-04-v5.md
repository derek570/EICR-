...
3a. Caller — locate the `buildResolvedBody({` call at line 564. The existing call uses **explicit `key: input.xxx ?? null` assignment style** for the input-derived keys, NOT object-shorthand. Preserve that style verbatim on every existing line; only the new `contextCircuits` line is added (mirroring the same `input.<snake_case> ?? null` shape):
...
WARNING — do NOT rewrite the existing explicit-assignment lines into shorthand (e.g. `contextField,`). Identifiers like `pendingWrite`, `contextField`, `contextCircuit` exist ONLY as `input.<snake_case>` in the outer scope; shorthand would produce a `ReferenceError` at first dispatch.
...
5d. Add a CORE DIRECTIVE so the XOR constraint is hard-stated, not only implied by the schema description. In the CORE DIRECTIVES section of `sonnet_agentic_system.md` (lines 32-36 — currently directives 1 through 5), append a new directive as item 6 (next free integer; no subsequent directives exist to renumber):
...
This closes the soft-contract concern: without a directive-level rule, Sonnet might silently set both fields and the validator's new XOR check (Step 4) would fire on legitimate attempts.
