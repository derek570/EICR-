...
| M1.5 | **Per-PDF inspector attestations.** Two checkboxes the inspector must tick at every certificate-issuance moment — once that they have reviewed every reading, once that they have reviewed every observation. Audit-logged with timestamp, user, job, PDF S3 key. Full spec at [pdf-issuance-attestations.md](./pdf-issuance-attestations.md). Note: this is principally a **professional-liability** mitigation (BS 7671 / inspector-responsible framing), not a data-protection control — but it is recorded here because it strengthens the M11.1 architectural mitigation and provides retrievable evidence of "meaningful human review" if an ICO inquiry ever questions the AI-assistance posture. | R1 (indirectly, via R11), R11 | Pending implementation — solicitor-text-final |
...
| **M11.1** | **Inspector-review-required architecture.** No certificate is issued until the inspector explicitly reviews and confirms each extracted field. Strengthened evidentially by **M1.5** (per-PDF attestations), which produces a durable audit-log row for every issuance. | R11 | In place — UI design choice; M1.5 audit-log layer pending implementation |
