...
| Before | After |
|---|---|
| "Deepgram Nova-3 has a 500-TOKEN limit" | "Deepgram Flux accepts a `keyterms` array via the WebSocket Configure message (max 100 per Flux docs; iOS's binding constraint is the 2000-char URL budget at `DEEPGRAM_MAX_URL_LENGTH` — `DeepgramService.swift:48`, with the rationale at lines 18-47; in the current bundled config this works out to a soft ceiling of ~95 keyterms — a heuristic for prompting Claude, NOT a hard server-side rejection threshold). Use 95 as the binding recommendation budget." |
...
- `scripts/generate-report-html.js` — category badge colours / labels for the new entries. Additionally, split the recommendations array on render: `codeRecommendations = recommendations.filter(r => r.category !== 'harness_probe')` (existing checkbox/accept/reject flow with `old_code`/`new_code` blocks) vs `probeRecommendations = recommendations.filter(r => r.category === 'harness_probe')` (new "Suggested regression probes" section, no accept/reject controls). `acceptSelected()` MUST only submit original indices for `codeRecommendations` — probe-only suggestions are never accepted as source edits. Apply analogous treatment to `flux_configure_keyterms_per_slot` recommendations (mark as `awaiting infrastructure`, no accept button).
- `scripts/session-optimizer.sh` — Pushover `REC_CAT` short-label switch at `case "$REC_CAT" in` ~line 1302.
- `scripts/__tests__/parse-optimizer-output.test.mjs` (and any `scripts/parse-optimizer-output.*` parser the test exercises) — grep for any closed allowlist of category strings and extend it to include the 8 new categories. Add at minimum one test fixture per new category to prove round-trip parsing.
...
- `focused_mode_timeline`: an array of `{at_ms, tool_call_id, slot_field, keyterm_count, enter_elapsed_ms, exit_reason}` rows pulled from `focused_mode_enter` / `focused_mode_enter_result` / `focused_mode_exit` events. Each row is one entry into a focused ask. **Telemetry prerequisite**: the current iOS log for `focused_mode_enter` carries only `toolCallId` and `sessionKeytermCount`; `inflight_question_anchored` carries no `field` / `circuit` / `toolCallId`. So `slot_field` cannot be reliably populated from the existing telemetry stream. Two paths: (a) extend `DeepgramRecordingViewModel.handleAlertTTSStarted` to include `field`, `circuit`, and `toolCallId` in both the local `inflight_question_anchored` debug log AND the `focused_mode_enter` / `focused_mode_enter_result` client diagnostics — preferred but requires iOS work this plan otherwise doesn't touch; or (b) emit `slot_field: null` in the timeline rows and document in the prompt that this field will become populated only after the iOS telemetry extension lands. The plan author SHOULD pick (a) and add a small iOS-side ticket alongside Item 6; if (b) is taken, the prompt's `flux_garble_single_word` detection signature loses its anchor and degrades significantly.
...
**Cluster 2 verification.** Force a re-process of yesterday's session 284CBBCD — by the time Cluster 2 lands, 284CBBCD is already in `processed_sessions` in `~/.certmate/optimizer_state.json`, so the optimizer will skip it unless re-injected. Either: (a) delete the `processed_sessions` entry for 284CBBCD via `jq 'del(.processed_sessions[] | select(test("284CBBCD")))' ~/.certmate/optimizer_state.json > .tmp && mv .tmp ~/.certmate/optimizer_state.json` then wait one poll cycle, or (b) use the existing audit/re-run mechanism in `session-optimizer.sh` (~line 2142 — the audit path that re-injects sessions via `first_seen`). Then confirm:
...
const KNOWN_BUG_SIGNATURES = [
  {
    ...
    detect: (analysis) => {
      // Match: an IR script `_completed` event whose values_snapshot
      // has a single-digit bare integer (1-9) on L-L or L-E,
      // alongside a sentinel value (>200) on the sibling slot. The
      // single-digit bare form is exactly the failure class commit
      // 3c77b1bb tightens (yesterday's L-L=2 fix).
      const events = analysis.dialogue_engine_transitions || [];
      return events.some(e =>
        e.schema === 'insulation_resistance'
        && e.event.endsWith('_completed')
        && (
             (Number.isInteger(Number(e.values_snapshot?.L_L)) && Number(e.values_snapshot.L_L) >= 1 && Number(e.values_snapshot.L_L) < 10
                && typeof e.values_snapshot?.L_E === 'string' && e.values_snapshot.L_E.startsWith('>'))
          || (Number.isInteger(Number(e.values_snapshot?.L_E)) && Number(e.values_snapshot.L_E) >= 1 && Number(e.values_snapshot.L_E) < 10
                && typeof e.values_snapshot?.L_L === 'string' && e.values_snapshot.L_L.startsWith('>'))
        )
      );
    },
  ...
  },
  {
    ...
    detect: (analysis) => {
      ...
      //
      // PREREQUISITE: extract KNOWN_FIELDS into a side-effect-free
      // module (e.g. `src/extraction/known-fields.js`) and expose the
      // iOS dual-alias allowlist from the same module or a new
      // `field-alias-allowlist.js`. Both sonnet-stream.js and this
      // analyzer file then import the constants. Without this step,
      // `analyze-session.js` cannot safely `require('sonnet-stream')`
      // (runtime side effects + WebSocket bootstrapping).
