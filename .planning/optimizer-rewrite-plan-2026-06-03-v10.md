...
- `scripts/session-optimizer.sh` download stanza (around the existing `aws s3 cp debug_log.jsonl` block) — add a matching `aws s3 cp ... backend_events.jsonl` with a fallback (e.g. `|| true` or `--no-error-on-missing`) so sessions recorded BEFORE the backend PR lands still process — they'll just produce empty `dialogue_engine_transitions` / `stage6_tool_calls` arrays rather than erroring.
