...
**Order-of-operations note:** the existing N/A short-circuit at lines 1183-1201 runs BEFORE this new block. For word-anchored fields whose schema INCLUDES `N/A` in options (none in the allowlist today), a reply of "N/A" / "not applicable" is caught there with the existing confidence and never reaches this block. For word-anchored fields WITHOUT `N/A` in options (the current allowlist), the N/A short-circuit's `field.options.includes("N/A")` gate is false and the flow falls through to this block — a reply of "na" then receives `invalid_value` with the full options list. That is the desired behaviour.
...
```
   - Add the same `buildWrites` helper directly after the guard. Same explicit-confidence rule as 2c — NO default; every existing branch keeps its prior value. **Verified confidences as of this plan (file is the canonical source — if it diverges, the file wins, re-audit before changing):**
     - Discontinuous-phrase branch (line 643): 0.9 → `buildWrites(value, 0.9)`.
     - Corrected-value `"0.7 no 0.47"` branch (line 683): 0.85 → `buildWrites(value, 0.85)`.
     - Single-numeric branch (line 704): 0.9 → `buildWrites(value, 0.9)`.
...
3c-iv. **CloudWatch observability for multi-circuit fan-out.** In the existing `logger.info('stage6.ask_user_enum_auto_resolved', { … })` block (around lines 771-780) and the matching `stage6.ask_user_value_auto_resolved` block (around lines 856-865), add a new `circuits` field next to the existing `circuit: contextCircuit`:

```js
circuits: input.context_circuits ?? null,
```
...
```js
// Top-of-file (one declaration, both resolvers reference):
import fieldSchema from '../../config/field_schema.json' assert { type: 'json' };
...
Verify the import path against `stage6-tool-schemas.js`'s own import of `field_schema.json` (it may use `require(...)` rather than `import ... assert`; mirror its pattern). Add tests covering at least `ze_at_db` (supply), `earth_loop_impedance_ze` (supply), and one `installation_details` field — each multi-circuit ask MUST short-circuit to `no_value_context`.
