...
- One-time cleanup of stuck harness entries in `~/.certmate/optimizer_state.json`. The state file mutates `first_seen`, `retry_counts`, and `processed_sessions` keys keyed by full S3 path; harness entries pollute the first two. Concrete one-shot:
  ...
  **Verification one-shot after applying both fixes**: `jq '.first_seen | keys[]' ~/.certmate/optimizer_state.json | grep harness_` should return empty. If it does not, the cleanup pass missed entries — re-run.
...
- `scripts/session-optimizer.sh` lines 1061-1086 and 1175-1199 — the Nova-style `TOKEN_BUDGET=450` / `KEYWORD_TOKENS` Python computation reads from `$IOS_DIR/Resources/default_config.json` (line 1064). This path is **stale**: the live bundled config is `$IOS_DIR/Sources/Resources/default_config.json` — fix the path AND the computation in one pass.
  ...
  Replace the Python computation with a simpler entry count: total keyterm count from the corrected JSON path, capped at 100 (Configure max), plus an estimated URL char cost (sum of `len(kw) + 9` chars per keyterm for the `&keyterms=` URL parameter overhead — the optimizer cannot compute the runtime URL exactly because it doesn't have iOS Configure-merge state, so the estimate is a static heuristic). **Drop `TOKEN_BUDGET` / `TOKEN_HEADROOM` entirely** — Flux has no token budget. Surface as new template vars: `{{KEYTERM_COUNT}}`, `{{KEYTERM_URL_CHARS_ESTIMATE}}`, `{{KEYTERM_CAP_REMAINING}}` (= 95 − count). Update the prompt heading at line 40 + the bullets to reference these new var names.
...
const KNOWN_BUG_SIGNATURES = [
  ...
  },
  {
    ...
    detect: (analysis) => {
      ...
      // rejection fired.
      //
      // IMPLEMENTATION TBD during execution. Structural skeleton:
      //   const readings = analysis.tool_calls?.filter(t => t.name === 'record_reading') || [];
      //   const rejections = analysis.tool_calls?.filter(t => t.name === 'value_not_in_options') || [];
      //   return readings.some(r => {
      //     const fieldEnum = CIRCUIT_FIELD_VALUE_ENUMS[r.args.field];
      //     if (!fieldEnum) return false;            // field has no enum yet
      //     if (fieldEnum.includes(r.args.value)) return false;
      //     return !rejections.some(rej => rej.args.field === r.args.field && rej.args.value === r.args.value);
      //   });
      // CIRCUIT_FIELD_VALUE_ENUMS must be imported from
      // src/extraction/stage6-dispatch-validation.js (already
      // side-effect-free; no refactor required).
    },
  ...
  },
  {
    ...
    detect: (analysis) => {
      ...
      //
      // IMPLEMENTATION TBD after prerequisite lands. Structural skeleton:
      //   const { KNOWN_FIELDS, IOS_DUAL_ALIAS_ALLOWLIST } = require('./known-fields.js');
      //   const FIELD_CORRECTIONS = require('../src/extraction/field-name-corrections.js').FIELD_CORRECTIONS;
      //   const leaks = (analysis.extraction?.readings || []).filter(r => {
      //     return !KNOWN_FIELDS.has(r.field)
      //         && !FIELD_CORRECTIONS[r.field]
