...
| **Steer 2 — observation C-coding deciding-facts checklist (id 83)** | Inside the existing **AMBIGUOUS C2/C3 SEVERITY** block (reuses its one-ask budget + `clarification_chain_id` framing — no parallel ask path) | A compact THREE-class checklist (enclosure penetrations/holes · basic protection at accessories/CUs · main protective bonding) that shares one ACCESS LADDER — live parts accessible TO TOUCH → **C1**; potential access → **C2**; tool/lockable or no danger → **C3/NC** — plus per-class BS 7671 citations (416.2.2 top-surface IP4X vs 416.2.1 side IP2X; 411.3.1.2 + 544.1.1/Table 54.8 bonding; 701.415.2 bathroom supplementary; 512.2 environmental IP ≠ 416.2). A FACT-WEIGHTING cue, never a blanket-default code: weight the deciding fact, ASK it first (the one-ask budget), then code. |

**Prompt-budget note:** the steers added ~+517 tokens (base render) / ~+516 (combined), minimised via the shared ACCESS LADDER consolidation. A measured cap bump on BOTH budget assertions in `src/__tests__/stage6-agentic-prompt.test.js` was the EXPECTED path (compaction cannot fold ~500 tokens away), not a fallback: base cap `renderedOn <= 16580` (measured 16477), combined cap `combinedRenderedOn <= 21820` (measured 21714), each `measured + ~100` headroom. One cold-cache window post-deploy (~5-min TTL re-warms).
...
**Key files:** `config/prompts/sonnet_agentic_system.md` (the two steers), `src/__tests__/stage6-agentic-prompt.test.js` (bumped budget caps + the Group 0b Steer-1 pin + the Group 18 Steer-2 pin, both asserted in BOTH flag renders), `src/__tests__/stage6-agentic-answers-session.test.js` (softened stale byte-identity comment).
