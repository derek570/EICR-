...
### PLAN-D 2026-08-23 addendum — terminal resolution, session ownership, obligation telemetry (feedback ids 127/130)

Plan-B's correlation machine above had a missing class of transition, found in the field (id 127): a correlation exit resolved ONLY the correlation state, never the SLOT machine, so a `.fastPlayed` slot stayed cocked and the value-blind slot gate swallowed the NEXT (different-value) same-slot confirmation as a "duplicate" — a dictated Zs correction applied on screen and was never spoken. Slot resolution on correlation-consume was a missing transition; it is now closed, for EVERY exit:

**Terminal-resolution model (iOS `AlertManager`).** The correlation → slot association is bound at DISPATCH time (`markFastPathPending(correlationId:slotKey:)` — a pending-time invalidation or TTL exit can fire before any playback-start could record it) and refreshed at playback start; the STORED key is authoritative (a current-board re-derivation after a mid-turn board switch would resolve the wrong slot). ONE resolver (`resolveFastPathCorrelationTerminal`) runs at every TERMINAL-RESOLUTION exit — correlated consume, mismatch invalidation, the end-of-envelope sweep (now also on nil/EMPTY-confirmations envelopes), TTL expiry (pending AND the new `.fastPlayed` TTL, same 300 s constant; route-local expiry transfers the arriving canonical into the fallback lifecycle rather than tombstoning before it plays), the queued-stale-clip check at queue start, playback-start parked-consume, session reset — leaving the correlation on a replay-suppressing `.resolved` tombstone and the slot `.resolved`. FAILURE is deliberately NOT resolver-terminal: it routes through the paired slot/correlation failure transitions and the FALLBACK-PENDING lifecycle (a parked canonical re-dispatches with its correlation identity intact; a late rejection is ownership-guarded so it can never reverse a tombstone). Per-exit reservation dispositions: covered exits CONVERT the parked key (heard via the fast clip); release exits DISCARD it (re-speakable); failure re-dispatches through a FALLBACK-PENDING lifecycle (the correlation keeps its identity while the fallback is queued — a wire replay parks instead of double-speaking; playback start terminalizes; every pre-play discard reopens). Routing returns a TYPED outcome (`.parked`/`.consumed`/`.notCorrelated`); the VM retains its open-correlation entry on `.parked` so the mismatch pre-scan can still invalidate before queued fast audio starts, and route-local TTL expiry also returns `.parked` (the machine transfers the arriving canonical into the fallback lifecycle and owns its dispatch). The HTTP rejection callback funnels through ONE atomic, correlation-owned transition (`handleFastAttemptRejection`) so a stale rejection can never reset a newer attempt's slot.

**Session-generation ownership.** `resetFastPathSessionState()` (invoked at recording start alongside `resetConfirmationDedupeStores`, and at teardown via `clearAll`) discards unreached parked reservations, tears down the whole TTS FIFO lifecycle, clears every fast-path store, and bumps `fastPathSessionGeneration`. Every deferred callback (the regexFastTTS HTTP completion, the pre-play failure Task, the fallback lifecycle closures) captures the generation at creation and fails CLOSED on mismatch; `playFastPathAudio` additionally requires an ACTIVE correlation (unknown/`.resolved`/`.fallbackPending` reject), so an old session's late response can never speak into a new session.

**Capability-conditioned obligation telemetry (`turn_audio_summary`, backend D4 — telemetry only, NO speech path).** The audio finalizer builds identity-matched audibility obligations from structured descriptors (one per fast correlation id — never from the slot-keyed identity map, which overwrites same-slot correlations — plus one per ACK-eligible bundler confirmation; a canonical carrying `fast_correlation_id` collapses with its fast attempt into ONE obligation retaining BOTH board aliases). ACKs reconcile correlation-first, unique-slot-second (circuit normalised `?? 0`); same-slot multiplicity classifies AMBIGUOUS (never exact completion or loss); completion is identity-matched, never count arithmetic; rejections keep exact correlation ids in both orderings (pre-arm consumed set; post-arm armed-finalizer update via `correlationToTurn` that removes only a twin's fast leg). Sessions whose capabilities lack `client_playback_telemetry` (web) classify UNOBSERVABLE — `expected_acks_eligible` forces 0 so the canonical row and the perceived-latency store agree, and their obligations are never reported unacked. The timeout row now names the exact unacked/ambiguous slot identities. The REAL audibility ledger (positive client NACK contract, replay-stable delivery tokens, reconnect leases) is a recorded follow-up — it needs a wire contract no 2026-08-23 wave sibling owns.

Tests: iOS `AlertManagerFastPathTerminalResolutionTests` (the 127 sequence, every exit, the fallback lifecycle, two-session reset), `AlertManagerFastPathCorrelationTests` + `DeepgramRecordingViewModelFastPathCorrelationTests` (typed outcomes, sweep/TTL exits); backend `voice-latency-turn-summary-obligations.test.js`.
...
**Morphological designation folding (id 116) + edge-token canonical matching (PLAN-B, ids 128/131, 2026-08-23).** `findCircuitsByDesignation` is TWO-PASS, and since PLAN-B both passes compare EDGE-CANONICALISED forms (standalone leading/trailing `circuit`/`circuits` tokens dropped via the shared `designation-canonicaliser.js` — the same helper the write dispatchers use): pass 1 compares the canonical stored form against the canonical QUERY in both substring directions (a reply that is nothing but banned tokens — "the circuit" — yields ZERO candidates; the old generic-word unique match is deliberately gone); pass 2 — consulted on zero pass-1 candidates AND whenever a pass-1 admission exists only through a newly-canonicalised variant (union + normal uniqueness, so a canonical pass-1 hit can never silently suppress the true pass-2 morphology target) — tokenises the ORIGINAL caller text into raw-offset records (leading filler AND edge banned tokens dropped by dropping records) and runs token-sequence containment through the CLOSED equivalence table (`light|lights|lighting → light`, `socket|sockets → socket`, `heater|heating → heater`) over the canonicalised stored tokens. Guards: a stored designation that canonicalises to EMPTY (bare "Circuit" — reachable via the persistence repair's banned-only-preserved rule) is excluded from designation matching entirely, raw and canonical, both passes; a short canonical remainder (<3 chars) matches token-boundary only; a single-letter/numeric-only remainder ("Circuit A" → "A") matches ONLY a bounded whole-designation reply or the explicit adjacency shapes "circuit A" / "the A circuit" / "A way" — fail closed. Metadata: `matchedDesignation` is now the CANONICAL variant (the literally-findable text `maskCircuitResolution` searches — returning the raw stored form would mask the whole reply and drop a co-dictated value); the RAW stored form stays on `sharedDesignation` for quote-back/display; a unique pass-2 match still returns `matchedUserSpan` (raw offsets), consumed BEFORE the literal search. The legacy IR/ring script twins mirror the pass-1 edge-token tolerance + exclusions only (no fold table); the answer-resolver's census is DECORATED (canonical value + eligibility flag, filtered only inside designation-matching lanes — broadcast/"circuit N"/multi-ref writes keep every row). No generic s/es suffix stripping (mangles "house"/"mains"), no edit-distance fuzz (banned).
...
- **Decline acks are the one documented exception to the exception (PLAN-G, 2026-08-14, id 114).** While confirmations-OFF suppresses ordinary reading confirmations, the P4 answered-ask DECLINE family ("Okay — leaving that one." etc.) always speaks regardless of the toggle — an answer to a question the app asked is not a reading confirmation, and silence after "don't worry" is indistinguishable from a broken pipeline. Backend: `stage6-shadow-harness.js`'s P4 net computes `isDecline` unconditionally and gates emission on `isDecline || confirmationsEnabled === true`. Web: `recording-context.tsx` forces this one family through `speakConfirmation` via `isP4DeclineAck()` (`web/src/lib/recording/confirmation-dedupe-key.ts`). Every OTHER P4 ack (the plain ANSWERED family) and all ordinary reading confirmations remain toggle-gated. **2026-08-14 (PLAN-G2, held finding 1):** iOS had the SAME local re-gate (`DeepgramRecordingViewModel.swift:10233`, `guard confirmationModeEnabled || confirmationAddressToken != nil else { continue }`); fixed via a byte-exact `isP4DeclineAck(_:)` predicate exempting matching confirmations from both the guard and its adjacent mode-off diagnostic-logging loop — all three clients now agree. PLAN-G2 also gave BOTH P4 ack families a replay-stable `dedupe_token: p4ack_<turnId>` (recognised via the same structural-prefix mechanism as `duplicate_`/`bulkoutcome_` on all three clients' dedupe-key builders), closing a pre-existing collision gap where two distinct P4 acks landing on the same rotated text within 30 s would silently swallow.
