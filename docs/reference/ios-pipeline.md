...
**What this buys standalone (no C4 machinery needed):** the detector's field label is only ever used as a boolean "is this a fresh structured reading?" guard — nothing downstream writes from the label itself. So even without the C4 semantic-slot ask-answer comparison, an ambiguous main-earth reply that arrives while an unrelated ask is pending simply fails that guard and falls through to the model as a fresh transcript, where the prompt ladder above resolves it. Two DECIDED-pin tests (`src/__tests__/stage6-pending-value.test.js`, `src/__tests__/stage6-dispatcher-ask-pending-value.test.js`) pin this standalone-safety claim.
