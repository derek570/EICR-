...
**2026-08-14 (PLAN-G2, held finding 2):** BOTH P4 ack families now carry a replay-stable `dedupe_token: p4ack_<turnId>`, structurally distinct from the other field-null apology families (which remain genuinely tokenless — the production site stamps the token, and the §A4 drain copies it through ONLY when the producer set one). A `field_null_fallback` fixture whose target IS a P4 ack MAY (and, where the value is deterministically computable, SHOULD) additionally assert `match.dedupe_token` — this strengthens the oracle from text-identity alone to text+turn-identity, closing the same collision class `matchAudibleOutputs` already treats seriously for the ask/ack pairing. The token is NOT hand-computable from the fixture alone in the recorded lane: it derives from `mintSessionId(corpusId)` (a deterministic hash) plus the turn number, so compute it via `node -e "import('./scripts/field-replay/lib/canonical-crypto.mjs').then(m=>console.log(m.mintSessionId('<corpus_id>')))"` then append `-turn-<N>`, or run the fixture once and read the emitted value from its pass output. The ack text must still be deterministic in the frozen lane — the P4 ack rotates `turnNum % ASK_DECLINE_ACK_PROMPTS.length` (turn 1 → "No problem, moving on."). Regression fixture (`is_keystone: false`): `frc_85ace7677…` (feedback id 85) RED-proves `audibility.output.out_decline_ack` on the net-reverted tree, and now also asserts the exact `p4ack_` token.
