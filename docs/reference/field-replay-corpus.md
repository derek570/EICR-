...
`turns[].confirmations_enabled` keeps the historical wire name so old captures remain byte-truthful. Under DictatedReadbackPolicyV1 it records the **Extra prompts** preference only. A false or absent value never permits an accepted dictated reading, correction, requested calculation, or reassignment to lose its mandatory read-back. Fixtures should therefore assert mandatory outcomes in both states; use the flag only when an optional prompt is the behavior under test.
...
The §A4-drained ack was originally **tokenless** (`field:null` is outside `DEDUPE_TOKEN_FIELDS`, so the emitted confirmation carried no `dedupe_token`), and runtime `confirmationMatches` keyed on `text_exact` with no token — so the fixture-schema validator admits a `field_null_fallback` matcher with a **non-empty trimmed `text_exact` alone** (the historical mandatory `dedupe_token`/`expected_key` was dropped for this kind; an empty/whitespace `text_exact` stays rejected — it is no meaningful oracle). This minimum is UNCHANGED — most field-null apologies (marker-①/②/F7) still carry no token at all.

**2026-08-14 (PLAN-G2, held finding 2):** BOTH P4 ack families now carry a replay-stable `dedupe_token: p4ack_<turnId>`, structurally distinct from the other field-null apology families (which remain genuinely tokenless — the production site stamps the token, and the §A4 drain copies it through ONLY when the producer set one). A `field_null_fallback` fixture whose target IS a P4 ack MAY additionally assert `match.dedupe_token` — this strengthens the oracle from text-identity alone to text+turn-identity, closing the same collision class `matchAudibleOutputs` already treats seriously for the ask/ack pairing. `semantic-judge.mjs`'s `field_null_fallback` branch was extended to compare it too (Codex diff-review cycle 1, PLAN-G2), so the token is enforced identically by BOTH consumers of a vendor-lane fixture — the field-replay recorded lane (`replay-assertions.mjs`) and the Plan-00 vendor-live semantic-oracle lane.
...
1. exactly one **cleaned designation projection** in `extracted_readings` (legacy fold name `designation`; the raw `circuit_designation` name space is scanned too) with `value` byte-equal to the op's `value`, and ZERO differing-value designation projections (the dirty write);
2. any legacy-shape `circuit_updates` projection for the circuit carries the cleaned designation;
3. the **post-turn AUTHORITATIVE stored designation** equals the cleaned value — read via the injected `readCircuitDesignation` (built over `getCircuitBucket` in `importExtractionModules`, **dynamically injected** after the fake-clock install exactly like `toClearWireField`; absent → the oracle latches INFRASTRUCTURE);
4. exactly one `circuit_op` state-change confirmation whose TRIMMED text byte-equals `confirmation_text_exact` (byte-exact deliberately: the clean "Upstairs lighting" IS a substring of the dirty "…Upstairs lighting circuit" read-back, so a contains-test false-passes on broken code);
5. when `no_ask_question_contains` is declared, ZERO emitted `ask_user_started` frames whose question contains it (the "Which circuit…?" ask-with-no-structural-gap class, Audio-First §2).

This exact shape exists because a naïve per-concern decomposition (a `reading` op + an audible-output matcher) REDs on pre-fix code with MULTIPLE independent failure ids (dirty projection, unmatched confirmation matcher, unclaimed-confirmation accounting) — violating the exact-`expected_failure_id` contract. Shape is fail-closed (`designation_hygiene_bad_shape`): singular non-null `circuit` (registered as created-this-turn for later ops), already-trimmed non-empty `value` that must ITSELF be edge-clean (a hygiene fixture may not lock a dirty expectation; the check uses the SAME closed delimiter set as `designation-canonicaliser.js` — hyphen/slash non-delimiters, so compounds pass and comma/period-joined edge tokens reject), already-trimmed `confirmation_text_exact`, `audibility: exactly_once`, no `circuits[]`/`field`/`state_transition`. When the op declares a `board_id`, EVERY leg is board-exact (projection, circuit_updates scan, stored-state read, confirmation count) — same-ref circuits on other boards are never inspected or double-counted; today's bundler emits circuit_op confirmations without a board key, so a board-scoped op's confirmation leg fails LOUD pending bundler board enrichment (deliberate: loud false-FAIL over silent cross-board false-PASS). The v1 rejection of bare `clear`/`rename`/`create_circuit` op kinds is unchanged. Regression fixture (`is_keystone: false`): `frc_6600a62a…` (2026-08-23 session, feedback id 128) — registered `required_green` under the fix-lands-first dual-proof rule with `red_proof_failure_id: designation_hygiene.op_desig_cc`; the RED half is EXECUTED by `designation-hygiene-red-proof.test.js` (canonicaliser mocked to pre-fix semantics → exactly that one id).

### The `dialogue_ingress` lane + `script_entry_resolution` assertion (PLAN-B id 131)

A turn may declare `dialogue_ingress: { family: insulation_resistance }` (closed enum): the runner then drives the REAL pre-harness dialogue-script entry — `processInsulationResistanceTurn`, the exact export sonnet-stream's `handleTranscript` calls BEFORE `runShadowHarness` — with the turn's transcript against the seeded session, using the production argument shape (raw reply = transcript on a non-answer turn; deterministic minted responseEpoch; the family wrapper is **dynamically injected** via `modules.dialogueScriptIngress`, never reimplemented). Such a turn carries NO `model_rounds`/`branches`/`ask_answers` (`dialogue_ingress_bad_turn` — a script-consumed turn never reaches the model; `srv-*` engine asks are answered by later transcripts, not the registry). The engine must CONSUME the turn (`handled && !fallthrough`) or the runner latches INFRASTRUCTURE.

Its paired atomic op, `kind: script_entry_resolution` (single failure id `script_entry_resolution.<operation_id>`, shape fail-closed via `script_entry_resolution_bad_shape`, `family` must match the turn's, state-dependent → prohibited under `empty_fallback`), jointly asserts: the engine `_entered` log row resolved the declared circuit **by designation** (`entry_designation_matched: true` — id 131's defect was `circuit_ref:null`), the dictated value WRITTEN to stored state (injected `readCircuitField`) AND emitted exactly once on the wire under its legacy reading-wire name (injected `toReadingWireField` — the reading-frame rename, deliberately not `toClearWireField`'s clear-dialect), ZERO which-circuit asks (`srv-…-which-…` id or question text), and EXACTLY ONE audible script ask which must be the declared next-slot ask (the value's own read-back is the script FINISH summary by design — beyond a single entry turn). Regression fixture (`is_keystone: false`): `frc_db9ad2a8…` — the 2026-08-23 session's IR entry with the dirty legacy designation SEEDED, single entry turn (a multi-turn walk REDs multi-id pre-fix because the which-circuit ask derails subsequent turns), registered `required_green` with `red_proof_failure_id: script_entry_resolution.op_ir_entry`; the RED half is EXECUTED by `designation-hygiene-red-proof.test.js` (canonicaliser mock reverts B3's matcher tolerance → exactly that one id through the same real engine path).
