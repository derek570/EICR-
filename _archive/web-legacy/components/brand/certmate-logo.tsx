...
/** Animated waveform bars — used in recording UI and hero graphics */
export function WaveformBars({
  ...
  return (
    <div className={cn('flex items-end gap-0.5', className)} style={{ height: 24 }}>
      {Array.from({ length: bars }).map((_, i) => (
        <div
          key={i}
          className={cn(
            'w-1 rounded-full transition-all duration-300',
            active ? 'bg-gradient-to-t from-blue-500 to-green-400 animate-pulse' : 'bg-white/20'
          )}
          style={{
            height: active ? `${40 + Math.sin(i * 1.2) * 40}%` : '20%',
