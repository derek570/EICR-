...
export const EXCLUDED_NOT_IN_REPLAY_CLOSURE = Object.freeze({
  ...
  // OpenAI extraction-provider trial (openai-tooluse-adapter.js). Reachable
  // ONLY when SONNET_EXTRACT_MODEL is a `gpt-*` model AND OPENAI_API_KEY is
  // set — the OpenAI-backed tool-call loop. The recorded replay lane pins
  // SONNET_EXTRACT_MODEL to the task-def Haiku value, so the OpenAI branch is
  // never taken during replay and this tuning knob is outside the closure.
  // (SONNET_EXTRACT_MODEL, OBSERVATION_EXTRACT_MODEL are PINNED_FROM_TASK_DEF;
  // OPENAI_API_KEY is a SECRET — both already classified above.)
  openai_trial: ['OPENAI_EXTRACT_REASONING_EFFORT'],
});
