#!/usr/bin/env node
/**
 * transcript-replay-direct-runner.mjs — the IMPLEMENTATION behind the thin
 * transcript-replay-direct.mjs bootstrap (field-replay corpus gate, plan
 * replay-corpus-gate-2026-07 Item 2). Carries the legacy voice-latency
 * scenario runner UNCHANGED (runLegacyCli — behaviour byte-for-byte for
 * --scenario/--scenario-dir invocations, incl. the AWS-fallback key loader
 * and the 50ms settle waits) plus the field-corpus lanes (runFieldCorpusCli
 * — recorded: fake clock + AWS deny + fetch deny + dummy key; live:
 * explicit-key-only + outbound-host policy + vendor-call cap).
 *
 * ZERO extraction modules in this module's static import graph — extraction
 * is dynamically imported per lane (the recorded lane installs the fake
 * clock + timer ledger FIRST).
 *
 * Original runner doc:
...
// Legacy CLI state — set by runLegacyCli(argv). Module-level `let` bindings
// (not const-at-parse) so the bootstrap can dispatch WITHOUT this module
// executing anything at import time; every function below reads the same
// names it always did, so legacy behaviour is byte-for-byte.
let SCENARIO_PATH = null;
let SCENARIO_DIR = null;
let FILTER = null;
let LANE = null;
let VERBOSE = false;
let LOADED_BARREL = true;
...
// --- Legacy main (voice-latency scenarios; behaviour unchanged) ----------
export async function runLegacyCli(argv) {
  ...
  SCENARIO_PATH = args.scenario ?? null;
  SCENARIO_DIR = args['scenario-dir'] ?? null;
  FILTER = args.filter ?? null;
  LANE = args.lane ?? null;
  VERBOSE = !!args.verbose;
  LOADED_BARREL = args['loaded-barrel'] !== 'off';
...
// --- Field-corpus lanes (plan Item 2) --------------------------------------
let _projectLogger = null;
let _removedTransports = [];
function restoreProjectLogger() {
  // Intentionally a no-op: the field-corpus CLI is a short-lived subprocess
  // whose ONLY stdout consumer is the JSON summary parser. Re-adding the
  // Console transport lets late async session-teardown log lines flush to
  // stdout AFTER the summary and corrupt it — the same failure the legacy
  // runner documents. The process exits immediately after, so there is no
  // cross-run leak to worry about.
  void _projectLogger; void _removedTransports;
}

export async function runFieldCorpusCli({ lane, argv }) {
  const args = Object.fromEntries(
    argv.map((a) => {
      const [k, v] = a.replace(/^--/, '').split('=');
      return [k, v ?? true];
  ...
  const corpusRoot = args.corpus ?? 'tests/fixtures/field-replay-corpus';
  const fixtureFilter = args.fixture ?? null;
  // --proof-state is structurally unavailable to the blocking package
  // script: it is honoured ONLY under --evidence-mode (a separate npm
  // script), and local proof runs are diagnostics, never evidence.
  const proofState = args['evidence-mode'] ? (args['proof-state'] ?? null) : null;

  const { runCorpus } = await import('../field-replay/lib/replay-runner-core.mjs');
  const loadFixture = (p) => yaml.load(fs.readFileSync(p, 'utf8'));

  // Wall clock captured BEFORE any fake clock installs — expiry/extension/
  // evidence checks use ONLY this value.
  const wallClockNowMs = Date.now();

  if (lane === 'recorded') {
    const { denyAwsCredentials } = await import('../field-replay/lib/aws-credential-deny.mjs');
    const { installRecordedFetchDeny } = await import('../field-replay/lib/network-guard.mjs');
    const { installReplayClock } = await import('../field-replay/lib/replay-clock.mjs');
    const FakeTimers = (await import('@sinonjs/fake-timers')).default;

    const restoreAws = denyAwsCredentials();
    const fetchGuard = installRecordedFetchDeny();
    // Corpus-lifetime clock + ledger, installed BEFORE the extraction-
    // dependent modules are dynamically imported (a per-scenario install
    // would come AFTER extraction-module evaluation, leaving early timers
    // unclassified). Recorded mode never calls getAnthropicKey()/
    // getElevenLabsKey() — the dummy key makes the AWS fallback
    // structurally unreachable.
    const clockCtl = installReplayClock(FakeTimers, { startMs: 1_000_000_000_000 });
    try {
      ...
      printCorpusSummary(summary, { lane, proofState });
      return summary.exitCode;
    } finally {
      clockCtl.uninstall();
      fetchGuard.restore();
      restoreAws();
      restoreProjectLogger();
    }
  }

  if (lane === 'live') {
    // EXPLICIT-KEY-ONLY loader: ANTHROPIC_API_KEY required, FAIL if absent —
    // the AWS-fallback loader is never called (a local live invocation on
    // the dev box would otherwise silently acquire and spend the
    // production key).
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) {
      console.error('field-replay live lane: ANTHROPIC_API_KEY is REQUIRED (explicit-key-only; the AWS Secrets Manager fallback is never used on this lane)');
      return 2;
    }
    ...
    try {
      ...
      const summary = await runCorpus({
        corpusRoot,
        modules,
        clockCtl: null, // live advisory lane keeps real timers
        loadFixture,
        proofState: null,
        fixtureFilter,
        wallClockNowMs,
      });
      summary.vendor_call_count = policy.state.vendorCalls;
      printCorpusSummary(summary, { lane });
      // Advisory only: report drift; never block a merge on it. Exit code
      // still reflects failures so the nightly can file its issue.
      return summary.exitCode;
    } finally {
      policy.restore();
      restoreProjectLogger();
  ...
  console.error(`unknown --model-lane value "${lane}" (recorded | live)`);
  return 2;
}

async function importExtractionModules() {
  // Detach the project logger's transports for the corpus run (snapshot-
  // and-restore per plan cleanup): winston Console output on stdout would
  // corrupt the JSON summary the caller parses — same failure class the
  // legacy runner documents. Restored by restoreProjectLogger().
  const projectLoggerModule = await import('../../src/logger.js');
  const projectLogger = projectLoggerModule.default;
  _removedTransports = [...projectLogger.transports];
  for (const t of _removedTransports) projectLogger.remove(t);
...
function printCorpusSummary(summary, { lane, proofState = null }) {
  const headline = summary.message
    ?? `${summary.passed}/${summary.executed} executed fixture(s) pass; ${summary.unsupported} unsupported_pending reported; ${summary.failed} failed`;
  process.stderr.write(`field-replay corpus [${lane}${proofState ? ` proof=${proofState}` : ''}]: ${headline}\n`);
  for (const r of summary.results ?? []) {
    process.stderr.write(`  ${r.verdict === 'pass' ? '\u2713' : r.verdict === 'reported' ? '\u25cb' : '\u2717'} ${r.corpusId}: ${r.verdict}${r.detail ? ` — ${r.detail}` : ''}\n`);
  }
