...
| Date | Summary |
|------|---------|
| 2026-08-06 | **Plan 00 evidence gate: `REQUIRED_DAYS` 3 → 1 (operator scripts + tests + docs; ZERO backend-behaviour and ZERO client-wire change; Derek).** The calendar-day spread was the wrong axis. A "day" in this gate is not a bucket of turns — it is the full ~10-requirement COVERAGE bundle (pinned IR × 5, both corpus lanes, a genuinely bound evidence-eligible field session, a Luna fast round, a Terra observation round, an explicit warm cache read, the dialogue-script and address-mirror families, and Derek's heard-it attestation) all landing inside ONE Europe/London day. Requiring that bundle three times bought REPETITION, not coverage, and charged for it by freezing the backend for three calendar days — any relevant source deploy voids the cohort and restarts the count. Derek is the sole user and has accepted the flake risk repetition would have covered; the coverage gate, which is the part that actually decides whether the Luna-vs-Haiku comparison is trustworthy, is UNCHANGED and still enforced per day, and evidence keeps accruing after DONE (DONE only unblocks downstream latency plans). **`0` is refused structurally, not by convention:** the fold's test is `acceptedDays.length >= REQUIRED_DAYS`, which at 0 is vacuously true — a cohort would reach DONE the instant it initialised with zero evidence, certifying the model comparison on no data at all — so `constants.mjs` now THROWS at module load if the constant is ever set below 1. The DONE-walk tests were made threshold-AGNOSTIC rather than re-pinned to new literals (a new `walkDays(count)` helper + assertions against `REQUIRED_DAYS`): pinning `'3/3'` had made the walk silently vacuous the moment the constant moved, and the test's real subject is the state machine — every day below the threshold HOLDs, the threshold day flips DONE. `fold.mjs`/`projections.mjs` needed no change (already parameterised); the per-day ✅/❌ requirement projection `status` writes beside the handoff is unchanged and is the monitoring surface. Also de-staled `deployment.md`: the runner note still claimed `run-ir`/`run-corpus` refuse unconditionally as mock/replay-only, which 00B-2/00B-3 (PRs #154/#157) retired — `refuseLiveDispatch()` is gone and a refusal now means an unmet live-lane precondition, not an unimplemented lane. Full detail: [changelog.md](docs/reference/changelog.md). |
