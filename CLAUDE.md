...
| Date | Summary |
|------|---------|
| 2026-09-22 | **Live extraction moved to `gpt-6-luna` (infra-from-source + adapter; backend only; ZERO wire/client change).** Half the price of `gpt-5.6-luna` on input and both cache buckets, 2.4x cheaper on output; the route is otherwise untouched. Three model-family tests were too narrow and every one fails SILENTLY — explicit prompt caching, cost rates, usage attribution. Carries NO latency or accuracy evidence: the recorded corpus replays a frozen model response and cannot supply one. Full detail: [changelog.md](docs/reference/changelog.md). |
