...
| Date | Summary |
|------|---------|
| 2026-08-17 | **Plan 08C-B (terminal-round lever) — CLOSED docs-only: feasibility gate = NO VIABLE MECHANISM; the 08-series latency campaign is COMPLETE (planning docs only; ZERO runtime diff).** All four content-independent transport axes are exhausted (tier already priority, prefix cache already free, connection already warm) or unavailable (predicted outputs: Chat-Completions-only, unsupported on gpt-5.x, decode-side while the terminal round decodes 4 tokens); the ~1.27 s zero-reasoning terminal round is the TTFT floor of a protocol-mandatory round. 08C-A's 1,332-turn bench replicates the shape within 1 ms of the field corpus. Audio-ready telemetry deliberately not shipped; contract preserved for a future separately-argued follow-up. Full detail: [changelog.md](docs/reference/changelog.md). |
