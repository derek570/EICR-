...
| Date | Summary |
|------|---------|
| 2026-08-11 | **Plan 08D closes docs-only — the discarded terminal round ships NO runtime early-release mechanism; the lever transfers to 08C (planning-docs only; ZERO `src/`/`web`/iOS diff; ZERO client-wire change).** Field measurement (12 turns, session `8B9B2BDD`, `eicr-backend:393`) resolves the terminal round's old fork to NEITHER branch — round-trip floor, 4 discarded tokens, 0 reasoning on 10/12 turns; 17 % (2/12) genuinely think and must never be silenced. The refine cycle (8 rounds, dual reviewer) then DISPROVED the "safe door" release mechanism at the transport level — the Responses adapter surfaces only `function_call` events, so terminality is knowable only at `response.completed`. With all five proposed mechanisms now dead, 08D ships nothing; 08C inherits the replication gate + four acceptance preconditions verbatim. This `/ep` run's own §4 recount over the combined 40-turn corpus: 0 correctives either corpus, bounds tighten to ≤~7.5 % of all turns / ≤~33 % of multi-round turns. Full detail: [changelog.md](docs/reference/changelog.md). |
