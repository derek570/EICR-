...
| Date | Summary |
|------|---------|
| 2026-08-25 | **PLAN-E1 (feedback-2026-08-23 wave) — PARTIAL: Deepgram key response gains a dark-shipped, session-latched `uplink_codec` field on both clients; the Opus sender/encoder itself is deliberately deferred (backend + web + iOS; ZERO behaviour change today).** E0's live probe confirms bare-Opus and ogg-Opus both decode correctly on Flux. Also fixes a live iOS bug: `AudioImportViewModel` now fetches a fresh `Bearer` grant per file instead of one expired `Token` JWT reused across a whole import loop. Full detail: [changelog.md](docs/reference/changelog.md). |
