...
| Date | Summary |
|------|---------|
| 2026-08-23 | **PLAN-D (feedback-2026-08-23 wave) — TTS exactly-once: the fast-path slot uncocks at every correlation exit (a swallowed Zs correction now speaks), and the create-op double ack merges into ONE clip (ids 127, 130; backend + iOS; web cured server-side).** iOS: dispatch-time correlation→slot binding + ONE terminal resolver at all eight exits (consume/invalidate/sweep-incl-empty-envelopes/TTL-incl-new-fastPlayed-TTL/failure/queued-stale-clip/parked-consume/session-reset), typed routing outcome (PARKED keeps the open entry), fallback-pending replay lifecycle, session-generation fencing so a stale HTTP response never speaks into a new session. Backend: create ack merges into the first non-fast carrier ("Created circuit 3, Downstairs light — wiring type A") via synthesis-time sidecars; all-fast/triple/bulk-disclosure/mid-stream-filter shapes covered; audio-finalizer telemetry becomes identity-matched obligations, capability-gated (web = UNOBSERVABLE, never unacked) — NO speech path added. Recorded fixture `frc_1e2f5aab…` locks the 130 shape. Full detail: [changelog.md](docs/reference/changelog.md). |
...
| 2026-08-17 | **Plan 08C-B (terminal-round lever) — CLOSED docs-only, NO VIABLE MECHANISM, confirmed by measurement; the 08-series is complete (planning docs + probe script + evidence; ZERO runtime diff).** Five transport axes dead: tier already priority, prefix cache engaged, connection warm, predicted outputs unavailable on gpt-5.x/Responses — and the Codex hold's one missed candidate, Responses-WS/`previous_response_id` chaining, was probed same-day and DISPROVED (n=12/arm A/B on `gpt-5.6-luna`: 205× smaller payload moved p50 first-text ~13 ms; the chained call still runs the full cached prefill). The ~1.27 s zero-reasoning terminal round is provider-side fixed overhead — the floor replicated within 1 ms across 08C-A's 1,332-turn bench. 08D §3 cross-record applied: four dead release shapes + five dead transport axes close both halves of the lever. Side-finding queued to Derek: `store` defaults TRUE on `/v1/responses`, so the org retains responses 30 days TODAY — `store:false` is a separate zero-latency privacy decision. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-17 | **Plan 08C-A (per-round cost §2.1) — MEASURED: NO ARM WINS, keep `recent_3` (benchmark harness + operator tooling; ZERO production behaviour change).** A 90-rep live-provider run (0 rate-limited, full coverage) found the snapshot circuit-ordering lever buys <4% round-stream time — under the +10% bar; `SNAPSHOT_RECENT_CIRCUITS` unchanged, eviction duplicate-write causality REFUTED, seams delivered dark (null defaults = byte-identical prod). Unblocks 08C-B. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-14 | **PLAN-G + PLAN-G2 — P4 decline acks bypass confirmations-OFF (id 114; backend + iOS + web).** PLAN-G shipped backend+web, iOS deferred; PLAN-G2 closed iOS + added a dedupe token. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-14 | **PLAN-F + PLAN-F2 (feedback-2026-08-11 wave) — spares in bulk apply, gate trigger vocabulary, all 6 CODEX-HELD findings resolved (ids 115, 78; backend + iOS + web).** Device-attribute fields (8-field OCPD+RCD union) include spares by default in bulk writes, reading fields exclude — an orthogonal `spare_policy` tool input composes with ANY scope (bulk, single, range), resolution mirrored on all 3 clients; skips are audible + count-aware. Plus `extent`/`limitation`/`limitations` join the weak-trigger gate vocabulary (13-month orphan, id 78). PLAN-F2 then closed PLAN-F's own 6 CODEX-HELD findings: composite `(callId, effectiveBoardId)` identity joins a bulk-outcome ledger entry to its own confirmation; LIM gate evaluates post-spare-filter candidates; multi-board omitted-`board_id` entries resolve against the effective board; new `bulkoutcome_` dedupe-token prefix across all 3 client/server layers. 5-cycle Codex review found+fixed a stray-comma parse bug and a stale-fallback-disclosure BLOCKER. PR #183 (EICR) + #62 (iOS) merged, TestFlight build 435 shipped. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-13 | **PLAN-C (feedback-2026-08-11 wave) — Deepgram Sleeping tier retired by default (id 120; iOS + web + docs; ZERO backend change).** Derek believed auto-sleep was already fully retired; the 60s no-transcript timeout was still live and a new wake-during-close race destroyed the 3s replay buffer twice in one field session (CF052DF3, 18 sleep cycles/68 min). Both clients now stream continuously; machinery stays config-gated behind a hidden, session-latched `autoSleepEnabled` flag (default off). C1a fixes a second iOS race in interruption-reconnect (generation-owned API); C2a routes ALL explicit pause callers (Pause button, BFCache, visibility-recovery) through an iOS-parity lighter-weight pause that keeps Sonnet alive — a real bug in this fix (Sonnet silently rebuilt on resume) was caught by this plan's own new tests. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-13 | **PLAN-B (feedback-2026-08-11 wave) — fast-path TTS exactly-once: correlation wire hop, slot identity, honest orphan net (ids 118, 119; backend + iOS + web).** A reading read back twice, a corrected reading never, and a false apology for an already-confirmed turn — all one iOS identity bug. Backend now stamps `fast_correlation_id` only on a value-and-text-exact match (never a correction); a fast-attempt ledger drives an honest orphan net; iOS's correlation state machine mirrors (never reverses) its existing slot machine. 20 real defects found+fixed by post-implementation Codex review (incl. two literal NUL bytes that made two backend source files register as binary). Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-13 | **PLAN-D — TTS language pin + confirmations-toggle audible-channel integrity (ids 121, 122, 124; backend + iOS + web).** ElevenLabs `language_code=en` pinned everywhere with a fail-open compatibility retry (fail-closed for the bench); confirmations-OFF is now the sole documented Audio-First exception — telemetered per-transcript, with a unified spoken cue on both toggle directions and a session-start warning, bound to the physical start only. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-12 | **PLAN-E (feedback-2026-08-11 wave) — the snapshot learns a job's existing installation address, and the postcode lookup stops manufacturing garbage (id 125; backend + iOS + web).** The snapshot never ingested `jobState.installation`/`installation_details`, so the lookup silently overwrote "Lower Earley" with the electoral ward "Hawkedon"; fixed at ingestion + mapping, and the postcode confirmation now speaks the effective town/county. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-12 | **PLAN-A2 — a per-run dictation-provenance ledger stops the RCD script reading back values nobody said while a genuine trip-time correction went unspoken (id 117; backend only; ZERO client-wire change).** `initScriptState` gains an ordered `operations` list marked at all write/drain sites; seeded-value skips canonicalise-compare instead of blanket-skipping a genuinely different value; the dispatcher pre-resolves per-turn winners so a synchronous finish knows bundler-vs-script ownership; one terminal-sink helper speaks every uncovered dictation before all 17 `clearScriptState` exits. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-12 | **PLAN-A (feedback-2026-08-11 wave) — the dialogue engine bridges morphology, reads compound IR entries, and takes no for an answer (ids 114/116/123; backend + prompt only; ZERO client-wire change).** "Upstairs lights" now matches "Upstairs Lighting": pass 1 keeps the substring test byte-for-byte; pass 2 is a CLOSED fold table (no suffix-stripping, no fuzz) whose unique match returns the raw user-text span, keeping a co-dictated "tested at 500" parseable (id-105 masking). "…greater than 299 live to live and live to earth" certifies BOTH legs — label-pair-first, one-qualified-candidate, whole-span circuit guard; the `>` survives. And a pending-value ask can be DECLINED: "Don't worry." resolves silently with exactly ONE P4 decline ack (never the apology); a per-generation fingerprint makes a same-turn re-ask structurally impossible. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-08-11 | **Plan 08D closes docs-only — the discarded terminal round ships NO runtime early-release mechanism; the lever transfers to 08C (planning-docs only; ZERO `src/`/`web`/iOS diff; ZERO client-wire change).** One-session field measurement (12 turns, session `8B9B2BDD`, `eicr-backend:392`) resolves the terminal round's fork to NEITHER branch — round-trip floor, 4 discarded tokens, 0 reasoning on 10/12 turns; 17 % (2/12) genuinely think and must never be silenced; ratios stay provisional pending replication. The refine cycle (8 rounds) then disproved the "safe door" release mechanism at the transport level — terminality is knowable only post-completion. Every proposed mechanism — prior ones plus this cycle's safe door — is now dead; 08D ships nothing, 08C inherits the replication gate + four acceptance preconditions verbatim. §4 recount over the combined 40-turn corpus: 0 correctives either corpus, bounds tighten to ≤~7.5 % of all turns / ≤~33 % of multi-round turns. Full detail: [changelog.md](docs/reference/changelog.md). |
