...
| 2026-08-13 | **PLAN-D — TTS language pin + confirmations-toggle audible-channel integrity (ids 121, 122, 124; backend + iOS + web).** ElevenLabs `language_code=en` pinned everywhere with a fail-open compatibility retry (fail-closed for the bench); SUPERSEDED 2026-09-07 by DictatedReadbackPolicyV1: confirmations-OFF was the sole documented Audio-First exception — telemetered per-transcript, with a unified spoken cue on both toggle directions and a session-start warning, bound to the physical start only. Full detail: [changelog.md](docs/reference/changelog.md). |
