...
|------|---------|
<<<<<<< HEAD
| 2026-09-23 | **`ocpd_bs_en` is free text on both clients, and max Zs keys on the standard as well as the type (web + iOS + two `src/export.js` entries).** A `BS 3871` printed on the device was undictatable, and dictated with the legacy type `2` it printed a BS 1361 figure. Full detail: [changelog.md](docs/reference/changelog.md). |
=======
