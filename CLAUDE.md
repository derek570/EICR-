...
| Date | Summary |
|------|---------|
| 2026-09-26 | **`packages/` is actually linted now, and CI runs the linter (tooling only; ZERO runtime change).** The 2026-09-22 fix only stopped `npm run lint` erroring; the shared wire-contract workspaces were still checked by nothing. The TS parser was already a devDependency and simply never wired up. `npm run lint` now runs inside the `Test Backend` required check, errors-only so it ratchets rather than being wrapped in `|| true`. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-09-26 | **PLAN-W2 — a locally parsed apply-field value is accepted exactly or handed to the model, never guessed (Decision 7 wrong-value wave; backend + web + iOS; ZERO wire change).** iOS stored the first word of a boolean ("polarity not correct" → `not`) and the first digit run of a number; web unit-stripped `30 mA` to `30 m`; both wrote the first circuit 3 on a two-board job. New shared contract `config/apply-field-value-vectors.json` (TestFlight preflight): a bare number plus its units, or one boolean token stored as `✓`/`✗` and read back as the word. A declined command writes nothing and speaks one lag line, hands off, or forwards (multi-board even with an ask open, Decision W-1.4); declined finals skip the regex. iOS board matcher needs every phrase token; the OCPD type twins stop joining "C 32" (B-138). Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-09-26 | **PLAN-W1 — no wrong value reaches the certificate from a backend guess (Decision 7; backend only; ZERO wire/prompt change).** The orphan net no longer writes a re-parsed reading the model didn't write, except the Decision W-3 interim case. One shared sole-value grammar gates the engine's bare-value fallback and the value and enum ask resolvers; an ambiguous named capture hands off. A "yes" board answer and a fuzzy designation escalate; IR leg assignment stops guessing. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-09-25 | **PLAN-CD — a dictated OCPD standard the client cannot read goes to the model instead of a scripted re-ask, unless a backend question is open (web + iOS; no backend behaviour change; ZERO wire change).** With no ask open the client writes nothing, speaks nothing and forwards an ordinary transcript through a local-only gate authority; with one open it keeps the single re-ask, because a forwarded utterance can be swallowed as that question's answer. The open-ask latch reads its per-class lifetimes from the new `config/ask-class-lifetimes-v1.json`, guarded as a named pre-TestFlight step. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-09-25 | **PLAN-C2 — any OCPD type is recorded as said, with one spoken advisory (backend + web + iOS; ZERO wire change).** `ocpd_type` was a nine-value select, so a BS 3871 Type 2, a BS 1361 Type II or a 60947-2 K curve could not be recorded (Decision 6). It is free text everywhere, canonicalised by one grammar pinned in `config/ocpd-type-suggestions.json`. An unknown or standard-incompatible type is written, read back once with "not a type I know" / "may not be right for BS EN 60898", marked in the grid and listed in the PDF preflight; never repeated for the same value. BS 1361 `1`/`2` print `I`/`II`. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-09-24 | **PLAN-CS — the backend accepts a free-text OCPD standard, and an RCD answer can no longer overwrite it on an RCBO (id 140, Decision 4; backend only; ZERO wire/client change).** `ocpd_bs_en` becomes `text` with manifest-backed suggestions; one `parseOcpdStandard` shape rule (`ocpd_standard_shape`) guards every write boundary, and the Levenshtein fallback is gone. The one ask after a rejection has its own resolver, which keeps a bulk answer's scope. Both RCBO BS slots are now asked, never named-extracted or mirrored. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-09-23 | **PLAN-D — hands-free pause and resume by voice (web + iOS; ZERO backend/wire change).** Saying "pause" stops INPUT and nothing else (Decision 8): mic and Deepgram stay live, a paused final reaches no write, chime or forward and gets a throttled still-paused cue, and in-flight speech is still spoken. "resume" or the **Resume** tap plays a new local tone, then "Carrying on — anything said while paused wasn't recorded." A tap drops every Flux turn that started before it (Decisions 34, 34a). Web gains iOS's post-TTS holding buffer; iOS's mode-status cues fall back to clips pre-rendered once per session. Shared fixture `config/voice-pause-vectors.json`. Merge gates: phone probe and AirPods smoke. Full detail: [changelog.md](docs/reference/changelog.md). |
| 2026-09-23 | **PLAN-B — the model owns every "I didn't understand" response; the ask budget is gone (backend only; ZERO wire change).** A no-write turn now ends with one fitting `answer_user`; the ask budget and restrained mode are removed (Decision 3) and a `repeat_ask` server note tells the model when its question has failed twice. Canned nets make one retry call first: the model picks closed codes and may quote the inspector verbatim, the server renders the line, and the canned string is the last resort. marker-② speaks a landed write's read-back instead of apologising. Full detail: [changelog.md](docs/reference/changelog.md). |
