...
| Date | Summary |
|------|---------|
| 2026-09-14 | **PLAN-F — the long free-text fields grow with what you dictate (web + iOS; ZERO backend/wire change).** *"The text box does not grow with the text and you have to slide along"*, and in landscape *"I can only see the first few words."* Both clients grew the BOX and not the INPUT: iOS wrapped a single-line `TextField` in `.frame(minHeight: 100)`, web rendered a fixed `rows={4}` textarea with `resize-none`. New `CMFloatingTextEditor` (vertical axis, `lineLimit(3...12)`, every property of the single-line component kept) and an opt-in `autoGrow` `MultilineField` with the same 3–12-line bounds, applied to the same four fields — Extent, Comments, Departures, Departure Details. A separate type, not a `multiline` flag, because `CMFloatingTextField` has 40 mostly single-line call sites. Bounds are inline styles, not `min-h-[..]` utilities, because Tailwind v4 emits nothing for an interpolated arbitrary value and the cap would silently not exist. The observation fields already grow and are untouched; the five Installation callers whose iOS counterparts grow become dated `partial` divergences (owner: Derek) rather than staying silently wrong. Neither unit runner can assert a rendered height, so structure is pinned on both clients — including a guard that fails any future `CMFloatingTextField` + `.frame(minHeight:)` pairing — and layout is pinned by a Playwright case at phone viewports in both orientations, red-proofed, green on chromium and webkit. Device smoke still outstanding. Full detail: [changelog.md](docs/reference/changelog.md). |
