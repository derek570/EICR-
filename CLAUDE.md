...
| Date | Summary |
|------|---------|
| 2026-08-13 | **PLAN-F (feedback-2026-08-11 wave) — spares in bulk apply + gate trigger vocabulary (ids 115, 78; backend + iOS + web).** "RCD type for all circuits is AC" silently excluded a spare on iOS; backend/web disagreed on the default too. Device-attribute fields (8-field OCPD+RCD union) now include spares by default, reading fields exclude — an orthogonal `spare_policy` tool input composes with any scope, resolution mirrored byte-for-byte on all 3 clients; skips are now audible + count-aware ("…skipping N spare ways"), contradictory modifiers consumed locally. Plus `extent`/`limitation`/`limitations` join the weak-trigger gate vocabulary (13-month orphan, id 78). Full detail: [changelog.md](docs/reference/changelog.md). |
