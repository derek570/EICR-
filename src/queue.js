...
export async function startWorker() {
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      const onReady = () => { clearTimeout(timeout); conn.off("error", onError); resolve(); };
      const onError = (err) => { clearTimeout(timeout); conn.off("ready", onReady); reject(err); };
    ...
    worker.on("completed", (job, result) => {
      ...
      }).catch((err) => logger.error("Push notification failed (completed)", { error: err.message }));
    });
      ...
      if (job?.data) {
        try {
          ...
          sendPushToUser(job.data.userId, {
            ...
            url: "/dashboard",
            tag: `job-${job.data.jobId}`,
          }).catch((pushErr) => logger.error("Push notification failed (failed)", { error: pushErr.message }));
        } catch (dbErr) {
          logger.error("Failed to update job status to failed", { jobId: job.data.jobId, error: dbErr.message });
        }
      }
    });
    ...
    logger.warn("Redis unavailable, queue disabled. Jobs will run in-process.", { error: err.message });
  }
}
...
export async function enqueueJob(userId, jobId, localDir = null) {
  ...
  try {
    ...
    const job = await queue.add(`process-${jobId}`, { userId, jobId, localDir }, {
      jobId,
    });
