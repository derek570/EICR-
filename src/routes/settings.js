 ...
 */
router.post("/inspector-profiles/:userId/upload-signature", auth.requireAuth, upload.single("signature"), async (req, res) => {
  const { userId } = req.params;
  const file = req.file;
...
async function loadRegulations() {
  ...
  const regPath = path.resolve(import.meta.dirname, "..", "..", "config", "bs7671-regulations.json");
