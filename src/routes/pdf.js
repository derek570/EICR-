    ...
    try {
      ...
      if (extractedContent) {
          ...
          JSON.stringify(
            {
              ...
              earthing_arrangement:
                supplyChars.earthing_arrangement || boardInfo.earthing_arrangement || '',
              ...
              nominal_voltage_uo:
                supplyChars.nominal_voltage_uo || boardInfo.nominal_voltage_uo || '',
              ...
              number_of_supplies:
                supplyChars.number_of_supplies || boardInfo.number_of_supplies || '',
              supply_polarity_confirmed:
                supplyChars.supply_polarity_confirmed ?? boardInfo.supply_polarity_confirmed ?? '',
              earthing_conductor_csa:
                supplyChars.earthing_conductor_csa || boardInfo.earthing_conductor_csa || '',
              earthing_conductor_material:
                supplyChars.earthing_conductor_material ||
                boardInfo.earthing_conductor_material ||
                '',
              main_bonding_csa: supplyChars.main_bonding_csa || boardInfo.main_bonding_csa || '',
              bonding_conductor_material:
                supplyChars.bonding_conductor_material ||
                boardInfo.bonding_conductor_material ||
                '',
              bonding_conductor_csa:
                supplyChars.bonding_conductor_csa || boardInfo.bonding_conductor_csa || '',
              ...
              bonding_structural_steel:
                supplyChars.bonding_structural_steel ?? boardInfo.bonding_structural_steel ?? '',
              ...
              main_switch_voltage:
                supplyChars.main_switch_voltage || boardInfo.main_switch_voltage || '',
              main_switch_current:
                supplyChars.main_switch_current || boardInfo.main_switch_current || '',
              rated_current:
                supplyChars.main_switch_current ||
                supplyChars.rated_current ||
                boardInfo.rated_current ||
                '',
              ...
              surge_status_indicator:
                supplyChars.surge_status_indicator || boardInfo.surge_status_indicator || '',
              earth_electrode_type:
                supplyChars.earth_electrode_type || boardInfo.earth_electrode_type || '',
              earth_electrode_resistance:
                supplyChars.earth_electrode_resistance ||
                boardInfo.earth_electrode_resistance ||
                '',
              ...
              agreed_limitations:
                extracted.installation_details?.agreed_limitations ||
                boardInfo.agreed_limitations ||
                '',
              agreed_with:
                extracted.installation_details?.agreed_with || boardInfo.agreed_with || '',
              operational_limitations:
                extracted.installation_details?.operational_limitations ||
                boardInfo.operational_limitations ||
                '',
            },
            null,
            2
        ...
        if (extracted.boards && Array.isArray(extracted.boards) && extracted.boards.length > 0) {
          ...
          logger.info('Written boards.json for multi-board PDF generation', {
            jobId,
            boardCount: extracted.boards.length,
          });
        }
        ...
        try {
          const installationJson = await storage.downloadText(
        ...
        try {
          ...
          if (supplyJson) {
            ...
            try {
              ...
              // No board_details.json written for this job yet, or it is not
              // valid JSON. Either way the supply block below is the first
              // writer, so an empty board is the correct starting point — this
              // must not abort the PDF.
            }
