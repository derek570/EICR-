  ...
  try {
    ...
    if (session.audioHoldBuffer) {
      ...
      logger.info("── FLUSHING HELD CHUNK ON SAVE ──", {
        sessionId, isStale, heldChunkIndex: held.chunkIndex,
      });
      try {
    ...
    logger.info(isStale ? "Auto-saving stale session" : "Finishing recording session", {
      sessionId, jobId, isExistingJob, isStale,
      address: jobAddress,
      ...
      try {
        ...
        if (gptFills > 0) {
          try {
            ...
            await storage.uploadText(JSON.stringify(enrichedData, null, 2), `${s3Prefix2}extracted_data.json`);
            await storage.uploadText(JSON.stringify(formData.supply_characteristics, null, 2), `${s3Prefix2}supply_characteristics.json`);
            await storage.uploadText(JSON.stringify(formData.installation_details, null, 2), `${s3Prefix2}installation_details.json`);
            await storage.uploadText(JSON.stringify(formData.observations, null, 2), `${s3Prefix2}observations.json`);
 ...
 */
router.post("/recording/:sessionId/chunk", auth.requireAuth, routeTimeout(60000), upload.single("audio"), async (req, res) => {
  ...
  try {
    ...
    if (audioBuffer.length < MIN_CHUNK_BYTES && !session.audioHoldBuffer) {
      ...
      logger.info("── SHORT CHUNK HELD ──", {
        sessionId, chunkIndex, audioSize: audioBuffer.length, threshold: MIN_CHUNK_BYTES,
      });
    ...
    if (session.audioHoldBuffer) {
      ...
      try {
        ...
        logger.info("── CHUNKS CONCATENATED ──", {
          sessionId, heldChunkIndex: held.chunkIndex, currentChunkIndex: chunkIndex, concatPath,
        });
      } catch (concatErr) {
        logger.warn("── CONCAT FAILED, using current chunk only ──", {
          sessionId, error: concatErr.message,
        });
        await fs.unlink(held.path).catch(() => {});
      }
    }
    ...
    session.debugLog.push({
      chunkIndex, chunkStartSeconds,
      timestamp: new Date().toISOString(),
      audioKey: debugAudioKey, audioBytes,
      wasConcatenated,
    ...
    logger.info("── TRANSCRIPTION RESULT ──", {
      sessionId, chunkIndex, modelUsed,
      attempts: transcribeResult?.attempts || 0,
    ...
    if (commonParsed.length > 0) {
      logger.info("── COMMON READINGS LOCAL PARSE ──", {
        sessionId, chunkIndex, readings: commonParsed,
      });
    }
    ...
    if (shouldExtractNow) {
      ...
      logger.info("── EXTRACTION RESULT (wider window) ──", {
        ...
        circuitDetails: (Array.isArray(chunkData.circuits) ? chunkData.circuits : []).map(c => ({
          ref: c.circuit_ref, name: c.circuit_designation,
          zs: c.measured_zs_ohm, ir: c.ir_live_earth_mohm,
          ocpd: c.ocpd_rating_a, ring_r1: c.ring_r1_ohm,
          ring_rn: c.ring_rn_ohm, ring_r2: c.ring_r2_ohm, r1r2: c.r1_r2_ohm,
        })),
        ...
        observationDetails: (Array.isArray(chunkData.observations) ? chunkData.observations : []).map(o => ({
          code: o.code, location: o.item_location,
      ...
      if (ringParsed.length > 0 && ringCircuitName) {
        ...
        logger.info("── RING VALUES INJECTED POST-EXTRACTION ──", {
          sessionId, chunkIndex, circuit: ringCircuitName, injected: ringParsed,
        });
      }
        ...
        for (const reading of commonParsed) {
          ...
          if (injected) {
            logger.info("── COMMON READING INJECTED ──", {
              sessionId, chunkIndex,
              reading: reading.name, field: reading.field,
              value: reading.value, target: reading.target, circuit: reading.circuitName,
            });
          }
        }
      }
      ...
      logger.info("── ACCUMULATED CIRCUITS ──", {
        ...
        circuits: session.accumulator.circuits.map(c => ({
          ref: c.circuit_ref, name: c.circuit_designation,
          ring_r1: c.ring_r1_ohm || '-', ring_rn: c.ring_rn_ohm || '-',
          ring_r2: c.ring_r2_ohm || '-', r1_r2: c.r1_r2_ohm || '-',
          zs: c.measured_zs_ohm || '-', ir: c.ir_live_earth_mohm || '-',
        })),
      });
    } else {
    ...
    if (session.jobId && extractedAddress && !session.addressUpdated) {
      try {
        ...
        logger.info("── JOB ADDRESS UPDATED ──", {
          sessionId, jobId: session.jobId, address: extractedAddress,
        });
      } catch (updateErr) {
    ...
    res.json({
      success: true, chunkIndex, formData,
      debug_mode: session.debugMode,
    });
  } catch (error) {
 ...
 */
router.post("/recording/:sessionId/photo", auth.requireAuth, upload.single("photo"), async (req, res) => {
  const { sessionId } = req.params;
  const session = activeSessions.get(sessionId);
  ...
  try {
    ...
    session.pendingPhotos.push({
      filename, buffer: photoBuffer, audioSeconds,
    });
    ...
    logger.info("Photo added to session", {
      sessionId, filename, audioSeconds,
    ...
    res.json({
      success: true, filename, audioSeconds, formData,
 ...
 */
router.post("/session/:sessionId/analytics", auth.requireAuth,
  upload.fields([
        ...
        try {
          ...
          if (rawTranscript.trim().length > 20) {
            ...
            logger.info("Whisper mode: extracted transcript for GPT session extraction", {
              sessionId, transcriptLength: rawTranscript.length,
            });
          }
        } catch (err) {
