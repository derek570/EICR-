...
describe('filterQuestionsAgainstFilledSlots', () => {
  ...
  test('drops question when slot is already filled in stateSnapshot', () => {
    ...
    const questions = [{ field: 'r1_r2', circuit: 2, type: 'unclear', heard_value: 'unclear' }];
    const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
  ...
  test('drops some questions, keeps others in a mixed batch', () => {
    ...
    expect(result.map((q) => `${q.field}:${q.circuit}`).sort()).toEqual(['postcode:null', 'zs:4']);
  });
  ...
  test('handles missing stateSnapshot gracefully', () => {
    const questions = [{ field: 'zs', circuit: 2, type: 'unclear' }];
    expect(filterQuestionsAgainstFilledSlots(questions, null, new Set(), sessionId)).toHaveLength(
      1
    );
    expect(
      filterQuestionsAgainstFilledSlots(questions, undefined, new Set(), sessionId)
    ).toHaveLength(1);
    expect(filterQuestionsAgainstFilledSlots(questions, {}, new Set(), sessionId)).toHaveLength(1);
  });

  test('returns input unchanged when questions is empty or missing', () => {
    const snapshot = { circuits: { 2: { r1_r2: 0.64 } } };
    expect(filterQuestionsAgainstFilledSlots([], snapshot, new Set(), sessionId)).toEqual([]);
    expect(filterQuestionsAgainstFilledSlots(null, snapshot, new Set(), sessionId)).toBeNull();
    expect(
  ...
  test('tolerates non-Set resolvedFieldsThisTurn (defensive)', () => {
    ...
    expect(filterQuestionsAgainstFilledSlots(questions, snapshot, undefined, sessionId)).toEqual(
      []
    );
    expect(filterQuestionsAgainstFilledSlots(questions, snapshot, null, sessionId)).toEqual([]);
    expect(filterQuestionsAgainstFilledSlots(questions, snapshot, ['r1_r2:2'], sessionId)).toEqual(
      []
    );
  });
  ...
  describe('type whitelist (codex round-2 blocker)', () => {
    ...
    test('keeps tt_confirmation even when slot is filled', () => {
      ...
      const questions = [{ field: 'earthing_arrangement', circuit: 0, type: 'tt_confirmation' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toHaveLength(1);
    });
    ...
    test('keeps observation_* questions even when slot is filled', () => {
      ...
      const questions = [{ field: 'polarity', circuit: 2, type: 'observation_confirmation' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toHaveLength(1);
    });
  ...
  describe('heard_value cross-reference across snapshot (session 0952EC64)', () => {
    test('drops null-field unclear question whose heard_value is stored elsewhere', () => {
      ...
      const questions = [{ type: 'unclear', field: null, circuit: -1, heard_value: '0.13' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toEqual([]);
    });
    ...
    test('normalises numeric forms: "0.130" heard_value matches 0.13 stored', () => {
      ...
      const snapshot = { circuits: { 4: { r1_plus_r2: 0.13 } } };
      const questions = [{ type: 'unclear', field: null, circuit: -1, heard_value: '0.130' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toEqual([]);
    });
    ...
    test('keeps null-field question when heard_value is NOT stored anywhere', () => {
      const snapshot = { circuits: { 4: { r1_plus_r2: 0.13 } } };
      const questions = [{ type: 'unclear', field: null, circuit: -1, heard_value: '0.99' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toHaveLength(1);
    });
    ...
    test('keeps out_of_range warning even when heard_value is stored (non-refill type)', () => {
      ...
      const snapshot = { circuits: { 2: { zs: 9999 } } };
      const questions = [{ type: 'out_of_range', field: 'zs', circuit: 2, heard_value: '9999' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toHaveLength(1);
    });
    ...
    test("keeps null-field question with no heard_value (filter can't help)", () => {
      ...
      const snapshot = { circuits: { 4: { r1_plus_r2: 0.13 } } };
      const questions = [{ type: 'unclear', field: null, circuit: -1, heard_value: null }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toHaveLength(1);
    });
    ...
    test('preserves ">200" semantic prefix (not the same as "200")', () => {
      ...
      const questions = [{ type: 'unclear', field: null, circuit: -1, heard_value: '>200' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toHaveLength(1);
    });
    ...
    test('matches across different circuits (the "don\'t know where it goes" case)', () => {
      ...
      const questions = [{ type: 'orphaned', field: null, circuit: -1, heard_value: '0.13' }];
      const result = filterQuestionsAgainstFilledSlots(questions, snapshot, new Set(), sessionId);
      expect(result).toEqual([]);
    });
  });
