...
describe('matchOperations write oracle', () => {
  ...
  test('exactly_once fails on a duplicate write to the same slot', () => {
    ...
    const fails = matchOperations([op], { result });
    expect(fails.some((f) => f.outcome === OUTCOME.FAIL && /exactly one/.test(f.message))).toBe(
      true
    );
  });

  test('a kind with no faithful oracle latches INFRASTRUCTURE (never a silent pass)', () => {
    const fails = matchOperations(
      [{ operation_id: 'c', kind: 'clear', audibility: 'exactly_once' }],
      { result: {} }
    );
...
describe('matchOperations — clear_then_write joint oracle', () => {
  ...
  test('RED (the T10 wipe): replacement present BUT the stale clear survived → one FAIL', () => {
    const result = {
      ...
      field_corrections: [
        { field: 'insulation_resistance_l_l', circuit: 3, reason: 'clear_reading', board_id: null },
      ],
    };
    const fails = matchOperations([ctwOp()], { result, toClearWireField });
    expect(fails).toHaveLength(1);
  ...
  test('board matrix RED: explicit/explicit matching stale clear → one FAIL', () => {
    const result = {
      extracted_readings: [
        { field: 'ir_live_live_mohm', circuit: 3, value: '100', board_id: 'main' },
      ],
      field_corrections: [
        {
  ...
  test('board matrix RED: replacement written on the WRONG board → replacement-missing FAIL', () => {
    const result = {
      extracted_readings: [
        { field: 'ir_live_live_mohm', circuit: 3, value: '100', board_id: 'sub-1' },
      ], // wrong board
      // no stale clear
    };
    const op = ctwOp({ board_id: 'main', clear_board_id: 'main' });
...
describe('matchToolExpectations', () => {
  ...
  test('a stub_ok row (no is_error) never satisfies dispatcher_expectation:accept when the real row errored', () => {
    ...
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_1' && f.outcome === OUTCOME.FAIL)
    ).toBe(true);
  });
  ...
  test('a structured validation_error {code} matches its declared reject code (no [object Object])', () => {
    const tc = {
      ...
      name: 'record_reading',
      input: {},
      schema_expectation: 'reject',
      dispatcher_expectation: 'reject',
      dispatcher_reject_code: 'circuit_not_found',
    };
    const captured = {
  ...
  test('schema and dispatcher failures use DISTINCT ids (never collapse into one expected_red id)', () => {
    ...
    const captured = {
      ...
      logRows: [
        {
          name: 'stage6_tool_call',
          meta: { tool_use_id: 'toolu_3', is_error: true, validation_error: 'x' },
        },
      ], // dispatcher:accept violated
    };
    const ids = matchToolExpectations(turnWith(tc), captured)
      .map((f) => f.id)
      .sort();
  ...
  test('validator unavailable latches infrastructure (cannot verify schema_expectation)', () => {
    ...
    expect(
      fails.some((f) => f.id === 'tool.schema.toolu_4' && f.outcome === OUTCOME.INFRASTRUCTURE)
    ).toBe(true);
  });
  ...
  test('an ABSENT authoritative row → infrastructure for accept (never a vacuous pass)', () => {
    ...
    const fails = matchToolExpectations(turnWith(tc), {
      validateToolInput: acceptValidator,
      logRows: [],
    });
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_5' && f.outcome === OUTCOME.INFRASTRUCTURE)
    ).toBe(true);
  });
  ...
  test('an ABSENT authoritative row → infrastructure for reject too (missing ≠ satisfied)', () => {
    ...
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_6' && f.outcome === OUTCOME.INFRASTRUCTURE)
    ).toBe(true);
  });
  ...
  test('a dispatcher-error-only thin row (no authoritative row) → infrastructure', () => {
    ...
    const captured = {
      ...
      logRows: [
        {
          name: 'stage6.tool_call',
          meta: { tool_call_id: 'toolu_7', outcome: 'dispatcher_error' },
        },
      ],
    };
    const fails = matchToolExpectations(turnWith(tc), captured);
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_7' && f.outcome === OUTCOME.INFRASTRUCTURE)
    ).toBe(true);
  });
  ...
  test('reject with a MISSING validation_error but a declared reject_code FAILS (cannot confirm)', () => {
    ...
    const fails = matchToolExpectations(turnWith(tc), captured);
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_8' && f.outcome === OUTCOME.FAIL)
    ).toBe(true);
  });
  ...
  test('reject_code compares EXACTLY (no substring collision)', () => {
    const tc = {
      ...
      input: {},
      dispatcher_expectation: 'reject',
      dispatcher_reject_code: 'not_found',
    };
    const captured = {
      validateToolInput: acceptValidator,
      logRows: [
    ...
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_9' && f.outcome === OUTCOME.FAIL)
    ).toBe(true);
  });
  ...
  test('ask_user with NO lifecycle evidence (not emitted, no terminal row) → infrastructure', () => {
    ...
    expect(
      fails.some(
        (f) => f.id === 'tool.dispatcher.toolu_ask' && f.outcome === OUTCOME.INFRASTRUCTURE
      )
    ).toBe(true);
  });
  ...
  test('ask_user accept SATISFIED when the ask was emitted', () => {
    ...
    const captured = {
      validateToolInput: acceptValidator,
      logRows: [],
      wsFrames: [{ type: 'ask_user_started', tool_call_id: 'toolu_ask2' }],
    };
  ...
  test('ask_user declared accept but GATED (answer_outcome ask_budget_exhausted) FAILS', () => {
    ...
    const fails = matchToolExpectations(turnWith(tc), captured);
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_ask3' && f.outcome === OUTCOME.FAIL)
    ).toBe(true);
  });
  ...
  test('ask_user declared reject but POSED (emitted) FAILS', () => {
    ...
    const captured = {
      validateToolInput: acceptValidator,
      logRows: [],
      wsFrames: [{ type: 'ask_user_started', tool_call_id: 'toolu_ask4' }],
    };
    const fails = matchToolExpectations(turnWith(tc), captured);
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_ask4' && f.outcome === OUTCOME.FAIL)
    ).toBe(true);
  });
  ...
  test('ask_user reject with the WRONG declared reject code FAILS', () => {
    ...
    const fails = matchToolExpectations(turnWith(tc), captured);
    expect(
      fails.some((f) => f.id === 'tool.dispatcher.toolu_ask5' && f.outcome === OUTCOME.FAIL)
    ).toBe(true);
  });
  ...
  test('EMISSION is authoritative: emitted + reject-listed terminal row → reject still FAILS', () => {
    ...
    const captured = {
      ...
      logRows: [
        { name: 'stage6.ask_user', meta: { tool_call_id: 'toolu_ask6', answer_outcome: 'gated' } },
      ],
    };
    expect(
      matchToolExpectations(turnWith(tc), captured).some((f) => f.outcome === OUTCOME.FAIL)
    ).toBe(true);
  });
  ...
  test('EMISSION is authoritative: emitted + post-emission outcome → accept PASSES', () => {
    ...
    const captured = {
      ...
      logRows: [
        {
          name: 'stage6.ask_user',
          meta: { tool_call_id: 'toolu_ask7', answer_outcome: 'transcript_already_extracted' },
        },
      ],
    };
    expect(matchToolExpectations(turnWith(tc), captured)).toHaveLength(0);
  ...
  test('non-ask: DUPLICATE authoritative rows → infrastructure (no silent first-match)', () => {
    ...
    const captured = {
      ...
      logRows: [
        {
          name: 'stage6_tool_call',
          meta: { tool_use_id: 'toolu_d', tool: 'record_reading', is_error: false },
        },
        {
          name: 'stage6_tool_call',
          meta: { tool_use_id: 'toolu_d', tool: 'record_reading', is_error: true },
        },
      ],
    };
    expect(
      matchToolExpectations(turnWith(tc), captured).some(
        (f) => f.outcome === OUTCOME.INFRASTRUCTURE
      )
    ).toBe(true);
  });
  ...
  test('non-ask: a same-id row for a DIFFERENT tool → infrastructure (identity enforced)', () => {
    ...
    const captured = {
      ...
      logRows: [
        {
          name: 'stage6_tool_call',
          meta: { tool_use_id: 'toolu_x', tool: 'record_observation', is_error: false },
        },
      ],
    };
    expect(
...
// ---------------------------------------------------------------------------
// A01P (2026-09-08) — `spoken_response` narration oracle. Matches the
// model-staged spoken answer (answer_user → result.spoken_response) by
  ...
  const expectation = (over = {}) => [
    {
      ...
      count: 1,
      match: { text_contains: 'LIM', text_not_contains: ['missing', 'not recorded'] },
      ...over,
    },
  ];
  const run = (spoken, outs = expectation()) =>
    matchAudibleOutputs(outs, {
      result: { spoken_response: spoken, confirmations: [] },
      wsFrames: [],
    });

  test('a compliant narration passes (contains LIM, never says missing)', () => {
    const { failures } = run("Ze is recorded as LIM, so I can't calculate Zs for circuit 1.");
    expect(failures).toEqual([]);
  });

  test('RED: "Ze is missing" fails the declared id', () => {
    const { failures } = run('I could not calculate Zs because Ze is missing.');
    expect(failures.map((f) => f.id)).toEqual(['audibility.output.out_lim']);
    expect(failures[0].outcome).toBe(OUTCOME.FAIL);
  });

  test('RED: "not recorded" fails; matching is case-insensitive', () => {
    const { failures } = run('Ze is NOT RECORDED on this board, LIM or otherwise.');
    expect(failures.map((f) => f.id)).toEqual(['audibility.output.out_lim']);
  });

  test('RED: no spoken_response staged at all fails with the no-answer message', () => {
    const { failures } = run(undefined);
    expect(failures).toHaveLength(1);
    expect(failures[0].message).toContain('no spoken_response');
  });

  test('count 0 asserts ABSENCE of a matching narration', () => {
    const outs = expectation({ count: 0, match: { text_contains: 'missing' } });
    expect(run('Ze is missing.', outs).failures.map((f) => f.id)).toEqual([
      'audibility.output.out_lim',
    ]);
    expect(run('Ze is recorded as LIM.', outs).failures).toEqual([]);
  });

  test('a spoken_response is NOT an unclaimed audible output when no expectation names it', () => {
    const { failures } = matchAudibleOutputs([], {
      result: { spoken_response: 'Some answer.', confirmations: [] },
      wsFrames: [],
    });
    expect(failures).toEqual([]);
  });
