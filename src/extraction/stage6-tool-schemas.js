...
const startDialogueScript = makeTool({
  ...
  description:
    "Trigger a structured slot-filling walk-through for a multi-step test the engine's regex did not catch (Deepgram garbled the trigger phrase, or the inspector paraphrased). The server initialises the script state, records any values provided in the same utterance (via `pending_writes`), and asks the inspector for the next missing slot. Idempotent — emits ok:true with status:'already_active' when a script is already running, so calling defensively is safe.\n\nWHEN TO USE (schema-dependent — read carefully):\n\n  ring_continuity / insulation_resistance — the inspector said a TRIGGER PHRASE for a multi-slot test. Use this tool EVEN WHEN the utterance also contains a value: put the value in `pending_writes` and the engine handles writing it AND walking the remaining slots. The trigger phrase ALONE (no value) is the obvious case; the trigger phrase + first value (e.g. 'insulation resistance live to live greater than 299 on the cooker') is the COMMON case — the inspector is going to dictate live-to-earth next and expects the prompt for it. If you call `record_reading` for the value instead, the engine never enters the script and the remaining slots stay unfilled — the exact failure mode of session 87D33579 (2026-05-25).\n\n  ocpd / rcd / rcbo — ONLY when the inspector explicitly opens the walk-through with no complete value (e.g. 'doing the OCPD now on circuit 4', 'set the RCD for circuit 2'). A SINGLE complete value ('Circuit N BS-EN 60898' / 'Circuit N type B' / 'Circuit N 32 amps' / 'Circuit N RCD type AC') is record_reading, NOT start_dialogue_script — the inspector has given a one-shot reading and expects it written immediately.\n\nGUARD: skip when the engine has already entered the script (visible in the cached state snapshot as an active script section, or as a recent slot-prompt like 'What's the live-to-earth?' / 'What are the lives?').\n\nGARBLE TOLERANCE — Deepgram routinely mishears 'insulation' as 'installation' (training-data favouring), 'International' (initial-syllable confusion — both /ɪn-/ openings, both end in -ation; session 87D33579), 'instellation', 'instance or', 'isolation', etc. If the value shape matches IR ('greater than N', 'N MΩ', 'live to live', 'live to earth') treat the trigger as IR even when the head word is garbled. Mirror logic for 'ring' → 'wing'/'rim' garbles.",
  properties: {
    schema: {
      type: 'string',
