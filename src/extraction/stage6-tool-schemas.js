...
const startDialogueScript = makeTool({
  ...
  description:
    "Trigger a structured slot-filling walk-through for a multi-step test the engine's regex did not catch (Deepgram garbled the trigger phrase, or the inspector paraphrased). Use ONLY when (a) the inspector EXPLICITLY opens a multi-slot walk-through (e.g. 'I'm doing the OCPD on circuit 4', 'ring continuity on circuit 2', 'insulation resistance now') AND there's NO single complete value in the SAME utterance, AND (b) the engine has not already entered it (no `Got it. R1 ... Rn ... R2` confirmation, no `What are the lives?` prompt visible, etc.). The server initialises the script state and asks the inspector for the next missing slot. Idempotent — emits ok:true with status:'already_active' when a script is already running, so calling defensively is safe. Do NOT use for one-shot readings (single Zs, single PFC, single BS-EN code, single curve letter, single amp rating, etc.) — those are record_reading. Specifically: 'Circuit N OCPD BS NNNN' / 'Circuit N MCB is BS-EN NNNN' / 'Circuit N type B' / 'Circuit N 32 amps' are ALL record_reading calls, NOT start_dialogue_script — the inspector has given a complete value and expects it written immediately, not a follow-up question.",
  properties: {
    schema: {
      type: 'string',
