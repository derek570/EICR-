...
export async function refineObservation(openai, obs, context = {}) {
  ...
  const prompt = [
    ...
    '- "Outdoor / outside / external" describes the LOCATION of the equipment,',
    "  not whether it's portable. A wall-mounted outdoor luminaire / garden",
    '  light drilled to a wall or post / soffit downlight / security light is',
    '  FIXED equipment, not "mobile equipment used outdoors".',
    '- Fixed outdoor luminaire (or any fixed luminaire indoors) without RCD',
    '  → schedule_item 5.12.4, regulation 411.3.4, code USUALLY C3',
    '  (improvement recommended); escalate to C2 only when there is a',
    '  separate aggravating factor (visible cable damage, water ingress,',
    '  mechanical risk exposure).',
    '- Fixed outdoor socket-outlet without RCD → schedule_item 5.12.1,',
    '  regulation 411.3.3, code C2 (since Amendment 2 2022).',
    '- "For the supply of mobile equipment ... for use outdoors" (5.12.2 /',
    '  411.3.3) applies ONLY to a circuit whose purpose is to supply',
    '  PORTABLE / MOBILE items (lawnmower, hedge trimmer, jet wash, power',
    '  tools). Do NOT use 5.12.2 just because the equipment is outdoors.',
    '- Test of intent: "is this equipment screwed/bolted/drilled into the',
    '  building permanently?" — if yes, it is fixed and routes via 5.12.1',
    '  (socket) or 5.12.4 (luminaire). If no, it is mobile and routes via',
    '  5.12.2.',
    '- When a sub-section under 5.12 reads as if it could apply (because the',
    '  wording mentions "outdoors"), check the OTHER 5.12.x sub-sections',
    '  first — there is almost always a more precise match.',
    '',
    '- schedule_item MUST be a value that appears verbatim in the appended',
    '  Schedule list. Do not invent sub-sections (no 5.12.5, no 5.18.10).',
    "  If you can't find a precise match, return null rather than guess.",
    '',
