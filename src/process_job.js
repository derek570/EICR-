  ...
  try {
    ...
    if (lastSelected) {
      ...
      if (found) {
        return {
          ...
          signature_file: found.signature_file
            ? path.join(projectRoot, 'assets', 'signatures', found.signature_file)
            : null,
        };
      }
    }
    ...
    if (defaultProfile) {
      return {
        ...
        signature_file: defaultProfile.signature_file
          ? path.join(projectRoot, 'assets', 'signatures', defaultProfile.signature_file)
          : null,
      };
    }
    ...
    return {
      ...
      signature_file: first.signature_file
        ? path.join(projectRoot, 'assets', 'signatures', first.signature_file)
        : null,
    };
  } catch {
    return {
...
async function maybeScalePhotos(jobDir, outDir) {
  ...
  try {
    ...
    if (validTimes.length >= 1) {
      ...
      manifest.spanMinutes = Math.round(
        (validTimes[validTimes.length - 1] - validTimes[0]) / 60000
      );
    }
...
function missingToMd(missing) {
  ...
  return (
    '# Missing values\n\n' +
    missing.map((m) => `- **${m.item || 'Unknown'}** — ${m.reason || ''}`).join('\n') +
    '\n'
  );
}
...
export async function processJob({ jobDir, outDir, dryRun = false, jobId: providedJobId = null }) {
  ...
  if (audioPaths.length > 0) {
    for (const audioPath of audioPaths) {
      ...
      const partTranscript =
        typeof t === 'string' ? t : t && typeof t.transcript === 'string' ? t.transcript : '';
      ...
      if (partTranscript.trim()) {
        ...
        const modelUsed =
          typeof t === 'object' ? t.modelUsed || null : process.env.GEMINI_MODEL || null;
    ...
    if (transcript) {
      await fs.writeFile(path.join(outDir, 'transcript.txt'), transcript, 'utf8');
    }
  }
  ...
  if (fssync.existsSync(scaledPhotosDir)) {
    ...
    if (photoResult.analysis) {
      ...
      await fs.writeFile(path.join(outDir, 'photo_analysis.txt'), photoAnalysis, 'utf8');
    ...
    try {
      ...
      if (recordingManifest.photos && recordingManifest.photos.length > 0) {
        ...
        const photoTimings = recordingManifest.photos
          .map((p, i) => {
            const formatted =
              p.audio_timestamp_formatted || formatAudioTimestamp(p.audio_timestamp_seconds);
  ...
  const questionsContext =
    questionsForInspector.length > 0
  ...
  const { csv, rows, observations, missing, board, installation, supply_characteristics } =
    extractResult;
  ...
  const mergedCsv =
    [csvHeaders.join(','), ...merged.map((r) => csvHeaders.map((h) => r[h] || '').join(','))].join(
      '\n'
    ) + '\n';

  await fs.writeFile(path.join(outDir, 'test_results.csv'), mergedCsv, 'utf8');

  /* 5a️⃣ Circuit mention detection */
  // Detect circuits mentioned in transcript but not extracted
  const mentionedCircuits = new Set(
    (transcript.match(/circuit\s*(\d+)/gi) || []).map((m) => m.match(/\d+/)?.[0]).filter(Boolean)
  );
  ...
  await generateTestResultsPDF({
    ...
    testDate: (enrichedInstallation?.date_of_inspection
      ? new Date(enrichedInstallation.date_of_inspection)
      : new Date()
  ...
  if (obsSettings.suppress_audio_quality_fi) {
    filteredObservations = filteredObservations.filter((obs) => {
      ...
      if (isAudioQualityFI) {
        log.debug('[OBSERVATION] Suppressed audio quality FI', {
          title: obs.title || obs.item_location,
        });
        return false;
      }
      return true;
  ...
  if (fssync.existsSync(scaledPhotosDir)) {
    const photoFiles = (await fs.readdir(scaledPhotosDir)).filter((f) =>
    ...
    if (photoFiles.length > 0 && observationsWithPhotos.length > 0) {
      observationsWithPhotos = observationsWithPhotos.map((obs) => {
        ...
        if (obs.source_photo && typeof obs.source_photo === 'string') {
          ...
          log.warn(`AI specified source_photo not found`, {
            observation: obs.title,
            source_photo: obs.source_photo,
          });
        }

        // No source_photo provided by AI - leave observation without photo
        // This is better than guessing wrong with keyword matching
        log.debug(`No source_photo from AI, leaving observation without photo`, {
          observation: obs.title,
        });
        return obs;
      });
  ...
  await fs.writeFile(path.join(outDir, 'missing_values.md'), missingToMd(missing), 'utf8');
  ...
  return {
    ...
    outputs: {
      ...
      questionsForInspector:
        questionsForInspector.length > 0 ? 'questions_for_inspector.json' : null,
    },
  };
}
