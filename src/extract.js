...
export async function extractAll(input) {
  ...
  const system = `Return STRICT JSON ONLY (no markdown, no prose).
Shape:
{
...
Example: If "=== Photo 2: IMG_4570.jpg ===" shows "open cable entry", create:
{
  ...
  "regs": ["522.8.1"],
  "confidence": 0.9,
  "source_photo": "IMG_4570.jpg"  ← CRITICAL: Always include this
}
...
1. Ring circuit too long/oversized:
   ...
   - Create observation: Ring final circuit exceeds recommended maximum length. Measured resistance of [X] ohms indicates circuit length exceeds 100m limit (max 0.7Ω for 2.5mm² T&E).
...
2. Zs exceeds maximum permitted:
   ...
   - Create observation: Earth fault loop impedance (Zs) of [X]Ω exceeds maximum permitted value of [Y]Ω for the protective device, compromising disconnection time.
   - Code: C2
   - Regulation: 411.4.4
   - Schedule item: 5.6
...
3. Insulation resistance low:
   ...
   - Create observation: Insulation resistance of [X] MΩ is below minimum acceptable value, indicating insulation deterioration.
   - Code: C2 (if <1MΩ) or C3 (if marginal)
   - Regulation: 643.3.2
   - Schedule item: 5.3
